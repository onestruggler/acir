...
--
-- Clifford-Lemmas: a complete decision procedure for equality of 2-qubit
-- Clifford operators, rewrite-clifford, built by bootstrapping: lemmas,
-- a rewrite system, more lemmas, a stronger rewrite system.  Ported from
-- the Agda code accompanying Bian and Selinger, "Generators and relations
-- for 2-qubit Clifford+T operators" (CC BY 2.0).
------------------------------------------------------------------------

{-# OPTIONS --without-K --safe #-}

module Examples.Groups.Clifford+T-2qubit.Clifford-Lemmas where
...
module Clifford where
  ...
  data CRel : Context Clifford+T.Generator where
    -- Monoidal structure.
    comm-W : ∀ g -> W • [ g ]ʷ === [ g ]ʷ • W ∈ CRel
    comm-H0-H1 : H0 • H1 === H1 • H0 ∈ CRel
    comm-H0-S1 : H0 • S1 === S1 • H0 ∈ CRel
    comm-S0-H1 : S0 • H1 === H1 • S0 ∈ CRel
    comm-S0-S1 : S0 • S1 === S1 • S0 ∈ CRel

    -- Order of elements.
    order-W : W ^ 8 === ε ∈ CRel
    order-H0 : H0 ^ 2 === ε ∈ CRel
    order-H1 : H1 ^ 2 === ε ∈ CRel
    order-S0 : S0 ^ 4 === ε ∈ CRel
    order-S1 : S1 ^ 4 === ε ∈ CRel
    order-S0H0 : (S0 • H0) ^ 3 === W ∈ CRel
    order-S1H1 : (S1 • H1) ^ 3 === W ∈ CRel
    order-CZ : CZ ^ 2 === ε ∈ CRel

    -- Remaining Clifford relations.
    comm-S0-CZ : S0 • CZ === CZ • S0 ∈ CRel
    comm-S1-CZ : S1 • CZ === CZ • S1 ∈ CRel
    rel-X0-CZ : X0 • CZ === CZ • S1 • S1 • X0 ∈ CRel
    rel-X1-CZ : X1 • CZ === CZ • S0 • S0 • X1 ∈ CRel
    rel-CZ-H0-CZ : CZ • H0 • CZ === S0 • H0 • CZ • S0 • H0 • S0 • S1 • W⁻¹ ∈ CRel
    rel-CZ-H1-CZ : CZ • H1 • CZ === S1 • H1 • CZ • S1 • H1 • S1 • S0 • W⁻¹ ∈ CRel

  -- Inclusion of the Clifford relations in the Clifford+T relations.
  inclusion-gen : ∀ {w v} -> w === v ∈ CRel -> Rel ⊢ w === v
...
module Clifford-Duality where
  ...
  lemma-double-dual ([ W-gen ]ʷ) = Eq.refl
  ...
  lemma-double-dual ([ T0-gen ]ʷ) = Eq.refl
  lemma-double-dual ([ T1-gen ]ʷ) = Eq.refl
...
module Clifford-Powers where
  ...
  lemma-comm-W {[ x ]ʷ} = axiom (Clifford.comm-W x)
  ...
  lemma-comm-W-power (suc zero) {g} = lemma-comm-W
  lemma-comm-W-power (suc (suc n)) {g} =
     equational (W • W ^ suc n) • g
             by assoc
         equals W • (W ^ suc n • g)
             by right lemma-comm-W-power (suc n)
         equals W • (g • W ^ suc n)
             by assoc reversed
         equals (W • g) • W ^ suc n
             by left lemma-comm-W
  ...
  step-order (W-gen ∷ []) = nothing
  step-order (W-gen ∷ W-gen ∷ t) = nothing
  step-order (W-gen ∷ xs) = just (xs ++ (W-gen ∷ []) , claim)
    where
      claim =
        equational W • word-of-list xs
            ...
            equals word-of-list xs • W • ε
                by lemma-append xs (W-gen ∷ [])
            equals word-of-list (xs ++ W-gen ∷ [])
...
module Clifford-Rewriting where
  ...
  rewrite-clifford : (n : ℕ) -> {w u : Word Generator} -> clifford-multistep n (list-of-word w) ≡ clifford-multistep n (list-of-word u) -> Rel ⊢ w === u
  ...
  clifford-tactic = rewrite-in-context clifford-multistep lemma-clifford-multistep-inclusion
    where
      lemma-clifford-multistep-inclusion : (n : ℕ) (xs : List Generator) -> Rel ⊢ word-of-list xs === word-of-list (clifford-multistep n xs)
