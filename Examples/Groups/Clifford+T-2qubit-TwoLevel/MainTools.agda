...
module Examples.Groups.Clifford+T-2qubit-TwoLevel.MainTools {n : ℕ} where
...
open import Examples.Groups.Clifford+T-2qubit-TwoLevel.Ring using (D ; Z ; oddᶻ)
...
open import Examples.Groups.Clifford+T-2qubit-TwoLevel.Column using (sylData ; nodd ; Above)
...
open import Examples.Groups.Clifford+T-2qubit-TwoLevel.Step using (step-lt)
open import Examples.Groups.Clifford+T-2qubit-TwoLevel.Synthesis using (synth-step)
open import Examples.Groups.Clifford+T-2qubit-TwoLevel.Derived {n} using (_⁻¹ ; inverseˡ ; idx ; Apartʷ ; Apartʷʷ ; comm-words)
...
open import Examples.Groups.Clifford+T-2qubit-TwoLevel.ExpLevel {n} using (word-level)
open import Examples.Groups.Clifford+T-2qubit-TwoLevel.Reduction {n}
...
------------------------------------------------------------------------
-- Paths of transpositions and ω's, and apartness
...
word-below-all [ ω-gen c ]ʷ h M lM lB = lM , word-level [ ω-gen c ]ʷ h M lM lB
...
-- A word acting on indices ≥ j is apart from ω_[a] for a < j.
above-apart : ∀ {j a : Fin n} (u : Word (Gen n)) → Above j u → a < j → Apartʷ u (ω-gen a)
...
above-apart [ H-gen b c p ]ʷ j≤b a<j =
  (≢ᵃ (ℕP.<-≤-trans a<j j≤b) ∷ []) ∷ (≢ᵃ (ℕP.<-trans (ℕP.<-≤-trans a<j j≤b) (recompute (b FinP.<? c) p)) ∷ []) ∷ []
  where
  ≢ᵃ : ∀ {x a : Fin n} → a < x → x ≢ a
  ≢ᵃ a<x x≡a = FinP.<-irrefl (≡.sym x≡a) a<x
above-apart [ ω-gen b ]ʷ j≤b a<j = (≢ᵃ (ℕP.<-≤-trans a<j j≤b) ∷ []) ∷ []
  where
  ≢ᵃ : ∀ {x a : Fin n} → a < x → x ≢ a
  ≢ᵃ a<x x≡a = FinP.<-irrefl (≡.sym x≡a) a<x
...
above-apart′ [ H-gen b c p ]ʷ j≤b h hs =
  All.map (≢< j≤b) hs ∷ All.map (≢< (ℕP.≤-trans j≤b (ℕP.<⇒≤ (recompute (b FinP.<? c) p)))) hs ∷ []
  where
  ≢< : ∀ {j b : Fin n} → j ≤ b → ∀ {y} → y < j → b ≢ y
  ≢< j≤b y<j b≡y = FinP.<-irrefl (≡.sym b≡y) (ℕP.<-≤-trans y<j j≤b)
above-apart′ [ ω-gen b ]ʷ j≤b h hs = All.map (≢< j≤b) hs ∷ []
  where
  ≢< : ∀ {j b : Fin n} → j ≤ b → ∀ {y} → y < j → b ≢ y
  ≢< j≤b y<j b≡y = FinP.<-irrefl (≡.sym b≡y) (ℕP.<-≤-trans y<j j≤b)
...
within-apart [ H-gen a b q ]ʷ b≤c h hs =
  All.map (≢> (ℕP.≤-trans (ℕP.<⇒≤ (recompute (a FinP.<? b) q)) b≤c)) hs ∷ All.map (≢> b≤c) hs ∷ []
  where
  ≢> : ∀ {x c : Fin n} → x ≤ c → ∀ {y} → c < y → x ≢ y
  ≢> x≤c c<y x≡y = FinP.<-irrefl x≡y (ℕP.≤-<-trans x≤c c<y)
within-apart [ ω-gen a ]ʷ a≤c h hs = All.map (≢> a≤c) hs ∷ []
  where
  ≢> : ∀ {x c : Fin n} → x ≤ c → ∀ {y} → c < y → x ≢ y
  ≢> x≤c c<y x≡y = FinP.<-irrefl x≡y (ℕP.≤-<-trans x≤c c<y)
...
-- A matrix that agrees with I beyond p, and whose column p is w / δᵏ
