...
module Examples.Groups.Clifford+T-2qubit-TwoLevel.Lde where
...
-- Odd and even elements of ℤ[ω]: the δ-residue is 1 or 0.
Odd Even : Z → Set
Odd w = oddᶻ w ≡ true
Even w = oddᶻ w ≡ false
...
-- A representation w / δᵏ is minimal if k = 0 or some entry is odd.
...
... | K₁ , z , x≡ | K₂ , ws , xs≡ =
  K , (δᶻ ^ᶻ (K ∸ K₁)) ZR.* z ∷ Vec.map ((δᶻ ^ᶻ (K ∸ K₂)) ZR.*_) ws ,
  cong₂ _∷_ (trans x≡ (raise K₁ (ℕP.m≤m⊔n K₁ K₂) z))
            (trans xs≡ (vec-ext λ y → begin
               ...
               sc K ((δᶻ ^ᶻ (K ∸ K₂)) ZR.* (ws ! y))           ≡⟨ cong (sc K) (sym (VecP.lookup-map y ((δᶻ ^ᶻ (K ∸ K₂)) ZR.*_) ws)) ⟩
               sc K (Vec.map ((δᶻ ^ᶻ (K ∸ K₂)) ZR.*_) ws ! y)  ≡⟨ sym (scV-! K (Vec.map ((δᶻ ^ᶻ (K ∸ K₂)) ZR.*_) ws) y) ⟩
               scV K (Vec.map ((δᶻ ^ᶻ (K ∸ K₂)) ZR.*_) ws) ! y ∎))
  where
  open ≡-Reasoning
  K = K₁ ⊔ K₂
  raise : ∀ k → k ≤ K → ∀ z → sc k z ≡ sc K ((δᶻ ^ᶻ (K ∸ k)) ZR.* z)
  raise k k≤K z = trans (sc-raise (K ∸ k) k z) (cong (λ e → sc e ((δᶻ ^ᶻ (K ∸ k)) ZR.* z)) (ℕP.m∸n+n≡m k≤K))

-- Dividing a vector of even entries by δ.
halveV : (w : Vec Z n) → (∀ x → Even (w ! x)) → Vec Z n
halveV w ev = tabulate (λ x → proj₁ (even⇒δ∣ (w ! x) (ev x)))

halveV-! : (w : Vec Z n) (ev : ∀ x → Even (w ! x)) (x : Fin n) → w ! x ≡ δᶻ ZR.* (halveV w ev ! x)
halveV-! w ev x = trans (proj₂ (even⇒δ∣ (w ! x) (ev x)))
                        (cong (δᶻ ZR.*_) (sym (VecP.lookup∘tabulate (λ x → proj₁ (even⇒δ∣ (w ! x) (ev x))) x)))
...
scV-halve k w ev = vec-ext λ x → begin
  ...
  sc (suc k) (δᶻ ZR.* (halveV w ev ! x))   ≡⟨ sc-δ k (halveV w ev ! x) ⟩
...
private
  -- A multiple of δ is even.
  δ*-even : ∀ y → Even (δᶻ ZR.* y)
  δ*-even y = δ∣⇒even (y , refl)
  ...
  dominated {k = k} {k'} {w} {w'} k<k' eq (inj₂ (x , odd)) = Odd⇒¬Even {w' ! x} odd w'x-even
    ...
    ... | suc e | _    | eq′ = trans (cong oddᶻ eq′)
                                     (trans (cong oddᶻ (ZR.*-assoc δᶻ (δᶻ ^ᶻ e) (w ! x))) (δ*-even ((δᶻ ^ᶻ e) ZR.* (w ! x))))
...
------------------------------------------------------------------------
-- The least δ-exponent and the numerator
...
-- The least δ-exponent is at most any δ-exponent.
