...
--
-- The expansions of X and ω are words of transpositions and ω's on
...
-- and (b + 1 , 0 , 1) lie below.  The expansions of H are conjugates
-- P Y P by such words P with P P = 1, around a single H; the states
...
module Examples.Groups.Clifford+T-2qubit-TwoLevel.ExpLevel {n : ℕ} where
...
open import Examples.Groups.Clifford+T-2qubit-TwoLevel.Scale using (sc ; sc-0 ; sc-injective ; λωᶻ)
...
open import Examples.Groups.Clifford+T-2qubit-TwoLevel.Norm using (lde0 ; Σℕ ; NA ; P)
open import Examples.Groups.Clifford+T-2qubit-TwoLevel.Residue using (δ²ᶻ)
open import Data.Integer.Base using (+_)
open import Examples.Groups.Clifford+T-2qubit-TwoLevel.Column using (nodd ; unit-odd)
...
open import Examples.Groups.Clifford+T-2qubit-TwoLevel.Syllable
  using (top ; Beyond-actM ; eᶻ ; eᶻ-! ; eδ-refl ; eδ-≢ ; col𝕀≡)
open import Examples.Groups.Clifford+T-2qubit-TwoLevel.Step using (col-norm)
open import Examples.Groups.Clifford+T-2qubit-TwoLevel.Levels using (mono-level ; mono-e ; <ₗ-trans ; Bℓ)
open import Examples.Groups.Clifford+T-2qubit-TwoLevel.Basic {n}
  using (Letters≤ ; HShape ; single ; sandwich ; expand ; expand-≈ ; expand-letters-X ; expand-letters-ω ; expand-shape-H)
open import Examples.Groups.Clifford+T-2qubit-TwoLevel.Reduction {n} using (BelowSrc ; ExpLevel ; sound-act)

------------------------------------------------------------------------
-- Along words of transpositions and ω's
...
word-level [ ω-gen c ]ʷ h M lM lB = mono-level (ω-gen c) tt h M lM lB
...
word-below [ ω-gen c ]ʷ _ M lM lB = lM
...
-- An H expansion from M to r: the states after the middle are P r.
kshape-below : ∀ {b : Fin n} {w : Word (Gen n)} → HShape b w → (M : Matrix n n D) {L : Lvl} →
...
private
  ...
  e-≢ᶻ {a} {x} x≢a = trans (eᶻ-! a x) (eδ-≢ x≢a)

  e-aᶻ : (a : Fin n) → eᶻ a ! a ≡ ZR.1#
  e-aᶻ a = trans (eᶻ-! a a) (eδ-refl a)

  -- H e_b at scale 2: entry a is λω.
  module He (a b : Fin n) .(p : a < b) where
    w = Hᶻ a b (eᶻ b)

    w-a : w ! a ≡ λωᶻ
    w-a = trans (set₂-a a b (λωᶻ ZR.* (eᶻ b ! a ZR.+ eᶻ b ! b)) (λωᶻ ZR.* (eᶻ b ! a ZR.- eᶻ b ! b))
                          (Vec.map (δ²ᶻ ZR.*_) (eᶻ b)))
                (trans (cong (λωᶻ ZR.*_) (trans (cong₂ ZR._+_ (e-≢ᶻ (<⇒≢ p)) (e-aᶻ b)) (ZR.+-identityˡ ZR.1#)))
                       (ZR.*-identityʳ λωᶻ))

    col≡ : actV (H-gen a b p) (col 𝕀 b) ≡ scV 2 w
    col≡ = trans (cong (actV (H-gen a b p)) (col𝕀≡ b)) (actV-H a b p 0 (eᶻ b))
...
top-moves (H-gen a b p) eq =
  λω≢0 (sc-injective 2 (trans chain (sym (sc-0 2))))
  where
  open He a b p
  chain : sc 2 λωᶻ ≡ DR.0#
...
top-moves (ω-gen a) eq =
  ω≢1 (trans (sym (DR.*-identityʳ ωᴰ))
        (trans (cong (ωᴰ DR.*_) (sym (δ-a a))) (trans (sym (actV-ia a (col 𝕀 a))) (trans (cong (_! a) eq) (δ-a a)))))
...
top-data (ω-gen a) {L} lt =
  subst (_<ₗ L) (cong₂ (λ k m → suc (toℕ a) , k , m) (proj₁ (mono-e (ω-gen a) tt a)) (proj₂ (mono-e (ω-gen a) tt a))) lt
top-data (H-gen a b p) {L} lt =
  <ₗ-trans (inj₂ (refl , subst (λ k → (0 , 1) <₂ (k , nodd (num (actV (H-gen a b p) (col 𝕀 b))))) (sym k2)
                             (inj₁ (ℕ.s≤s ℕ.z≤n)))) lt
  where
  open import Examples.Groups.Clifford+T-2qubit-TwoLevel.Pivot using (_<₂_)
  open He a b p
  k2 : lde (actV (H-gen a b p) (col 𝕀 b)) ≡ 2
  k2 = proj₁ (lde-char 2 w col≡ (inj₂ (a , cong oddᶻ w-a)))
...
private
  ...
  nodd-unit {M} o p k0 =
    at (lde0 (num (col M p)) (subst (λ k → + Σℕ (λ x → NA (num (col M p) ! x)) ≡ P k) k0 (col-norm o p)))
    where
    open import Data.Product.Base using (∃)
    at : (∃ λ m → _) → nodd (num (col M p)) ≡ 1
    at (m , (t , t<8 , um) , rest) =
      count-one (λ x → oddᶻ (num (col M p) ! x)) m (trans (cong oddᶻ um) (unit-odd t t<8))
    ...
    by (just p) em = cmp (FinP.<-cmp t p)
      ...
      cmp (tri≈ _ refl _) = exp (lde (col M p)) refl
        ...
        exp (suc k) ek = <ₗ-trans (subst (Bℓ t <ₗ_) (sym lvl)
          ...
          where open import Examples.Groups.Clifford+T-2qubit-TwoLevel.Pivot using (_<₂_)
      cmp (tri> _ _ p<t) =
...
exp-level (H-gen a b p) M o L lM lg =
  kshape-below (expand-shape-H a b p) M lM
    (subst (_<ₗ L) (cong level (sound-act (expand-≈ (H-gen a b p)) M)) lg)
    (B-bound (H-gen a b p) M o lM lg)
exp-level (ω-gen a) M o L lM lg =
  word-below (expand (ω-gen a)) (expand-letters-ω a) M lM (B-bound (ω-gen a) M o lM lg)
