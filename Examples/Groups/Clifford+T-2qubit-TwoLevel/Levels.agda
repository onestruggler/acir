...
--
-- Levels under the monomial generators X_[a,b] and ω_[a], towards the
-- fact that expanding a generator into basic ones does not raise the
-- level (§3.3): X and ω permute the entries of a column and multiply
-- them by units, so they keep its least δ-exponent and its
-- number of odd entries.
------------------------------------------------------------------------

{-# OPTIONS --without-K --safe #-}

module Examples.Groups.Clifford+T-2qubit-TwoLevel.Levels where
...
-- ω multiplies an entry by a unit, which keeps its parity.
odd-ω : (a : Fin n) (w : Vec Z n) (x : Fin n) → oddᶻ (ωᵛ a w ! x) ≡ oddᶻ (w ! x)
odd-ω a w x = at (x FinP.≟ a)
  where
  at : Dec (x ≡ a) → oddᶻ (ωᵛ a w ! x) ≡ oddᶻ (w ! x)
  at (yes refl) = trans (cong oddᶻ (set₁-a x (ωᶻ ZR.* (w ! x)) w)) (oddᶻ-* ωᶻ (w ! x))
  at (no xa) = cong oddᶻ (set₁-≢ a (ωᶻ ZR.* (w ! a)) w xa)

nodd-ω : (a : Fin n) (w : Vec Z n) → nodd (ωᵛ a w) ≡ nodd w
nodd-ω a w = count-cong (λ x → oddᶻ (ωᵛ a w ! x)) (λ x → oddᶻ (w ! x)) (odd-ω a w)

-- A standard basis vector has one odd entry.
nodd-e : (c : Fin n) → nodd (eᶻ c) ≡ 1
nodd-e c = count-one (λ x → oddᶻ (eᶻ c ! x)) c
  (trans (cong oddᶻ (trans (eᶻ-! c c) (eδ-refl c))) refl)
  (λ x x≢c → trans (cong oddᶻ (trans (eᶻ-! c x) (eδ-≢ x≢c))) refl)

------------------------------------------------------------------------
-- The least δ-exponent
...
Minimal-ω : ∀ {k} (a : Fin n) (w : Vec Z n) → Minimal k w → Minimal k (ωᵛ a w)
Minimal-ω a w (inj₁ k0) = inj₁ k0
Minimal-ω a w (inj₂ (x , ox)) = inj₂ (x , trans (odd-ω a w x) ox)

-- X and ω keep the exponent, and act on the numerator.
...
lde-ω : (a : Fin n) (v : Vec D n) →
        lde (actV (ω-gen a) v) ≡ lde v × num (actV (ω-gen a) v) ≡ ωᵛ a (num v)
lde-ω a v = lde-char (lde v) (ωᵛ a (num v))
  (trans (cong (actV (ω-gen a)) (lde-eq v)) (actV-ω a (lde v) (num v)))
  (Minimal-ω a (num v) (lde-min v))
...
mono-col (ω-gen a) _ v =
  proj₁ (lde-ω a v) , trans (cong nodd (proj₂ (lde-ω a v))) (nodd-ω a (num v))
...
-- X and ω are invertible, by their orders.
private
  ...
  ω⁸-act : (a : Fin n) (v : Vec D n) → actVʷ (ω a ^ 8) v ≡ v
  ω⁸-act a v = same-action (ω a ^ 8) ε (sound-axiom order-ω) v
...
mono-inj (ω-gen a) _ {u} {v} eq =
  trans (sym (ω⁸-act a u))
    (trans (cong (actVʷ (ω a ^ 7)) eq) (ω⁸-act a v))
