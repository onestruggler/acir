...
--
-- Completeness, reduced to the Main Lemma (§3.4).
--
...
-- generators (Lemma 3.4), which are proved by well-founded induction
-- on the level of the source (Lemma 3.5), each induction step being an
...
module Examples.Groups.Clifford+CS-TwoLevel.Reduction {n : ℕ} where
...
open import Examples.Groups.Clifford+CS-TwoLevel.Pivot using (pivot ; pivot-nothing ; Lvl ; level ; _<ₗ_ ; <ₗ-wellFounded)
open import Examples.Groups.Clifford+CS-TwoLevel.Syllable using (syl ; step)
open import Examples.Groups.Clifford+CS-TwoLevel.Synthesis using (synth ; synth-step)
open import Examples.Groups.Clifford+CS-TwoLevel.Derived {n} using (•-cancelˡ)
open import Examples.Groups.Clifford+CS-TwoLevel.Basic {n} using (IsBasic ; Basicʷ ; expand ; expand-≈ ; expand-basic)
