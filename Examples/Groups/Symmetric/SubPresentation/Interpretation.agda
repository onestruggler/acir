...
--
-- Soundness of the circuit presentation of Sₙ in the endofunction
-- semantics: the monoid congruence _≈_ generated by _VRel,_===_
-- preserves the denotation ⟦_⟧ : Word (Gen n) → Endo n.
--
-- Half of the setoid embedding that SubPres records; the other half is
-- completeness, which comes from the normal form.
------------------------------------------------------------------------

{-# OPTIONS --cubical-compatible --safe #-}

module Examples.Groups.Symmetric.SubPresentation.Interpretation where
...
sound-ax (cong↑ {_} {w₀} {v₀} p) k =
  Eq.trans (⟦↑⟧ w₀ k)
  (Eq.trans (shift-cong (sound-ax p) k) (Eq.sym (⟦↑⟧ v₀ k)))
sound-ax (comm₁ () _)            _
...
sound {n} p = go p
  ...
  go (cong {w₀} {_} {_} {v₁} p q) k =
    Eq.trans (go q (⟦ w₀ ⟧ k)) (Eq.cong ⟦ v₁ ⟧ (go p k))
  go assoc                        k = refl
  go left-unit                    k = refl
