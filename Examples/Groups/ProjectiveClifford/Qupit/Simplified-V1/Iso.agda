...
module Examples.Groups.ProjectiveClifford.Qupit.Simplified-V1.Iso
  (p-3 : ℕ)
  (let p-2 = ₁₊ p-3)
...
open import Examples.Groups.ProjectiveClifford.Qupit.Simplified-V1.Forward p-3 p-prime g* g-gen
open import Examples.Groups.ProjectiveClifford.Qupit.Simplified-V1.LemmasCZ p-3 p-prime g* g-gen hiding (module CL ; module CLb)
...
-- The isomorphism
--
-- Assembled from the two directions: f from Forward, h from
-- Iso-Inverse-Direction above.  The two `left-inv-gen` lemmas say that
-- each is the other's inverse on generators, which is all
-- StarGroupIsomorphism needs.
...
module M (n : ℕ) where
  ...
  open Iso n using (f ; h ; f-well-defined ; lemma-f*-SD↑)
  open Iso-Inverse-Direction n using (h-well-defined ; h-X ; h-Z ; hs ; lemma-h↑)
  ...
  f-left-inv-gen {₁₊ m} Cli.S-gen = begin
      [ Cli.S-gen ]ʷ
        ≈⟨ sym claim ⟩
      C.Z^ Cli.-1/2 • C.R
        ≡⟨ Eq.cong (λ z → z • C.R) (Eq.sym (lemma-fʷ-w^n {f = f} {w = SemiDirect.Z} (toℕ Cli.-1/2))) ⟩
      (f ʷ) (h Cli.S-gen) ∎
    where
    ...
    Zmod a = begin
      ...
      C.Z ^ (a % p Nat.+ a / p Nat.* p)                ≈⟨ ^-+ C.Z (a % p) (a / p Nat.* p) ⟩
      C.Z ^ (a % p) • C.Z ^ (a / p Nat.* p)            ≈⟨ cright (refl' (Eq.cong (C.Z ^_) (NP.*-comm (a / p) p))) ⟩
      C.Z ^ (a % p) • C.Z ^ (p Nat.* (a / p))          ≈⟨ cright (sym (^^ C.Z p (a / p))) ⟩
      C.Z ^ (a % p) • (C.Z ^ p) ^ (a / p)              ≈⟨ cright (^-cong (C.Z ^ p) ε (a / p) lemma-order-Z) ⟩
      C.Z ^ (a % p) • ε ^ (a / p)                      ≈⟨ cright (ε^k=ε (a / p)) ⟩
      C.Z ^ (a % p) • ε                                ≈⟨ right-unit ⟩
      C.Z ^ (a % p) ∎
    zhalf : C.Z^ Cli.-1/2 • C.Z^ Cli.1/2 ≈ ε
    zhalf = begin
      C.Z^ Cli.-1/2 • C.Z^ Cli.1/2                          ≈⟨ sym (^-+ C.Z (toℕ Cli.-1/2) (toℕ Cli.1/2)) ⟩
    ...
    claim : C.Z^ Cli.-1/2 • C.R ≈ Cli.S
    claim = begin
      C.Z^ Cli.-1/2 • C.R                                ≈⟨ cright commSZ ⟩
