...
module Examples.Groups.ProjectiveClifford.Qupit.Paper-V1.Syntactics
  (p-3 : ℕ)
  (let p-2 = ₁₊ p-3)
...
open Symplectic hiding
  ...
  ; M-mul ; semi-MS
  ; srel ; cong↑ ; comm₁ ; comm₂ ; lemma-cong↑ ; SHS) public
...
module Clifford-Relations where
  ...
  SHS : ∀ {n} -> ℤ* ₚ ->  Word (Gen (₁₊ n))
  SHS g*@(g , nz) = SHS' g g⁻¹
    where
    g*⁻¹ = g* ⁻¹
    g⁻¹ = g*⁻¹ .proj₁

  -- M is the filling with the unit outside and its inverse inside — the
  -- S-spelled copy of Paper-V0's RHR x x⁻¹ — and XM is the other way
  -- round, which is SHS at the unit itself.
  --
  -- M is written over SHS' directly rather than as SHS (x' ⁻¹).  The two
  -- are the same word, but SHS (x' ⁻¹) reaches its inner exponent as
  -- x' ⁻¹ ⁻¹, and every later step then has to discharge that double
  -- inversion; since ⁻¹ computes a Bézout witness, leaving it in place
  -- makes even XM≡M⁻¹ take minutes to check.  Collapsing it here keeps
  -- the exponents literally x and x⁻¹ throughout.
  M : ∀ {n} -> ℤ* ₚ -> Word (Gen (₁₊ n))
  M x' = SHS' ((x' ⁻¹) .proj₁) (x' .proj₁)

  XM : ∀ {n} -> ℤ* ₚ -> Word (Gen (₁₊ n))
  XM x' = SHS x'


  -- The two fillings are exchanged by inversion, the S-spelled copy of
  -- Gates' XM≡ZM⁻¹ and the same one-line proof as Paper-V0's: the inner
  -- exponents agree on the nose and the outer two differ by x ⁻¹ ⁻¹, so
  -- it is one congruence in the first argument of the shape.  (Not refl:
  -- inv-involutive equates the proj₁ of two units, not the units, so it
  -- cannot be pushed through SHS itself.)  This is the bridge between
  -- the axioms, which are stated over XMg, and the Mg forms that
  -- everything downstream is written in.
  XM≡M⁻¹ : ∀ {n} (x : ℤ* ₚ) → XM {n} x ≡ M (x ⁻¹)
  XM≡M⁻¹ x = Eq.cong (λ a → SHS' a ((x ⁻¹) .proj₁)) (Eq.sym (inv-involutive x))
  ...
  --
  -- X • Z === Z • X is NOT among them: it is derived in Lemmas.XZ as
  -- lemma-comm-X-Z, from order-H read as an equation for a Pauli.
  --
  -- order-SH — Figure 1's (S • H) ^ 3 = ε — is NOT among them either,
  -- and this is where Paper-V1 has one axiom fewer than Paper-V0.  Under
  -- the SHS' spelling below, M₁ *is* the word (S • H) ^ 3, so M-power at
  -- k = 0 already says it is ε; see Lemmas.OneWire.lemma-order-SH.  Under
  -- Paper-V0's R-spelling M₁ is (R • H) ^ 3 instead, a different word,
  -- which is why the axiom has to stay on that side.
  module Base where
    infix 4 _SRel,_===_
