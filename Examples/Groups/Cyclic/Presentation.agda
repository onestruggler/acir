...
------------------------------------------------------------------------
-- The group presentation, at positive order
...
presentation {n} = isPresentationOf subpresentation claim
  ...
  -- The section [_] of the normal form realises every element:
  -- ⟦ [ y ] ⟧ ≡ y.  At order 1 (ℤ/1ℤ) this is trivial; at order ≥ 2
  -- it is pow-id.
...
private
  module MS₀ =
    MonoidSem (0 Cn,_===_) (Eq.setoid (SN.NF 0)) NP.+-0-monoid (λ _ → 1)
...
private
  module Sub₀ =
    MS₀.GetSubPresentation (λ { order → refl }) (SN.nfp' 0) unique-nf₀
...
monoid-presentation = isMonoidPresentationOf Sub₀.monoidSubPres claim
  ...
  claim k =
    SN.[_] {0} k , λ {z} hyp → Eq.trans (Sub₀.fʷ-cong hyp) (sem-ℕ k)
