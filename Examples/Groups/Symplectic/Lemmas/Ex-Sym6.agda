...
module Examples.Groups.Symplectic.Lemmas.Ex-Sym6 (p-2 : ℕ) (p-prime : Prime (2+ p-2))  where
...
lemma-CX^aS^k''' a-1@(₁₊ a-2) k* = begin
  ...
  (S ^ k[a-1] • (S ^ k[a-1]) ↑ • CZ ^ -k[a-1]) • (S ^ k[a-1]) ↑ • S ↑ ^ (k[a-1] Nat.* 2) • (S ↑ ^ (-k[a-1] Nat.* 2) • CX • CZ ^ -k[a-1]) • CX ^ a-1 ≈⟨ (cright cright  by-passoc (□ • □ ^ 3 • □) (□ ^ 2 • □ ^ 2 • □) auto) ⟩
  ...
  (S ^ k[a-1] • (S ^ k[a-1]) ↑ • CZ ^ -k[a-1]) • (S ^ k[a-1]) ↑ • (CX • CZ ^ -k[a-1]) • CX ^ a-1 ≈⟨ (cright  by-passoc (□ • □ ^ 2 • □) (□ ^ 2 • □ ^ 2) auto) ⟩
  (S ^ k[a-1] • (S ^ k[a-1]) ↑ • CZ ^ -k[a-1]) • ((S ^ k[a-1]) ↑ • CX) • CZ ^ -k[a-1] • CX ^ a-1 ≈⟨ (cright cleft sym (aux-comm-CX-S^k↑-ℕ  k[a-1])) ⟩
  (S ^ k[a-1] • (S ^ k[a-1]) ↑ • CZ ^ -k[a-1]) • (CX • (S ^ k[a-1]) ↑) • CZ ^ -k[a-1] • CX ^ a-1 ≈⟨ by-passoc (□ ^ 3 • □ ^ 2 • □ ^ 2) (□ ^ 4 • □ ^ 3) auto ⟩
  ...
  where
    ...
    aux0 = begin
      ...
      S ↑ ^ ((1 Nat.+ p-1) Nat.* k[a-1]) ≈⟨ sym (^^ (S ↑) p k[a-1]) ⟩
      (S ↑ ^ p) ^ k[a-1] ≈⟨ ^-cong ((S ↑ ^ p)) ((S ^ p) ↑) k[a-1] (refl' (aux-↑ S p)) ⟩
      (S ^ p) ↑ ^ k[a-1] ≈⟨ ^-cong ((S ^ p) ↑) (ε ↑) k[a-1] (axiom (cong↑ order-S)) ⟩
      ε ↑ ^ k[a-1] ≈⟨ ε^k=ε k[a-1] ⟩
      ε ∎
    aux : S ↑ ^ (k[a-1] Nat.* 2) • S ↑ ^ (-k[a-1] Nat.* 2) ≈ ε
    aux = begin
      S ↑ ^ (k[a-1] Nat.* 2) • S ↑ ^ (-k[a-1] Nat.* 2) ≈⟨ sym (^-+ (S ↑) (k[a-1] Nat.* 2) (-k[a-1] Nat.* 2)) ⟩
      S ↑ ^ (k[a-1] Nat.* 2 Nat.+ -k[a-1] Nat.* 2) ≈⟨ refl' (Eq.cong ((S ↑) ^_) (Eq.sym (NP.*-distribʳ-+ 2 k[a-1] -k[a-1]))) ⟩
      S ↑ ^ ((k[a-1] Nat.+ -k[a-1]) Nat.* 2) ≈⟨ sym (^^ (S ↑) (k[a-1] Nat.+ -k[a-1]) 2) ⟩
