...
module Examples.Groups.Symplectic.Lemmas.Lemmas-2Qupit-Sym (p-2 : ℕ) (p-prime : Prime (2+ p-2))  where
...
module Lemmas-2Q (n : ℕ) where
  ...
  lemma-CZ^k+l k l = begin
    CZ^ k • CZ^ l ≈⟨ refl ⟩
    CZ ^ toℕ k • CZ ^ toℕ l ≈⟨ sym (^-+ CZ (toℕ k) (toℕ l)) ⟩
  ...
  lemma-semi-HH↓-CZ = bbc CZ (CZ^ ₋₁) aux
    ...
    aux = begin
      ...
      (CZ • CZ^ ₋₁) • H ↓ ^ 2 • CZ^ ₋₁ ≈⟨ by-passoc (□ ^ 2 • □ ^ 2) (□ • □ ^ 2 • □) auto ⟩
      CZ • (CZ^ ₋₁ • H ↓ ^ 2) • CZ^ ₋₁ ∎
  ...
  lemma-semi-HH↑-CZ^-₁ = begin
    H ↑ ^ 2 • CZ ≈⟨ lemma-semi-HH↑-CZ ⟩
    CZ^ ₋₁ • H ↑ ^ 2 ≈⟨ cleft refl' (Eq.cong CZ^ aux-₋₁=-1) ⟩
    CZ^ (- ₁) • H ↑ ^ 2 ∎
    where
    open SR word-setoid
    open Lemmas0 (₁₊ n)
  ...
  lemma-semi-CZ-HH↓' = begin
    H ↓ ^ 2 • CZ^ (- ₁) ≈⟨ cright sym (refl' (Eq.cong CZ^ aux-₋₁=-1)) ⟩
    H ↓ ^ 2 • CZ^ (₋₁) ≈⟨ sym lemma-semi-CZ-HH↓ ⟩
    CZ • H ↓ ^ 2 ∎
    where
    open SR word-setoid
    open Lemmas0 (₁₊ n)
