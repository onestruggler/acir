...
-- semi-M𝑠 / semi-M↑CZ / semi-M↓CZ / order-H, by collecting the Z^½
-- Paulis out of the 𝑠-powers (S,Z commute) and pushing them through
...
module Examples.Groups.Symplectic.Clifford.Mg-Simplify
  (p-3 : ℕ)
  (let p-2 = ₁₊ p-3)
...
module Push (n : ℕ) where
  ...
  Zmod k = begin
    ...
    Z ^ (k % p) • (Z ^ p) ^ (k / p)     ≈⟨ cright (^-cong (Z ^ p) ε (k / p) (CL.lemma-order-Z n)) ⟩
  ...
  Xmod k = begin
    ...
    X ^ (k % p) • (X ^ p) ^ (k / p)     ≈⟨ cright (^-cong (X ^ p) ε (k / p) (CL.lemma-order-X n)) ⟩
  ...
  ZcommS j m = comm⇒pow-comm (toℕ j) m (CL.lemma-comm-Z-S n)
  ...
  Spush1 = sym (begin
    ...
    ((X • Z) • S) • Z ^ p-1  ≈⟨ assoc ⟩
    (X • Z) • (S • Z ^ p-1)  ≈⟨ cright (comm⇒pow-comm 1 p-1 (sym (CL.lemma-comm-Z-S n))) ⟩
    (X • Z) • (Z ^ p-1 • S)  ≈⟨ assoc ⟩
    X • (Z • (Z ^ p-1 • S))  ≈⟨ cright (sym assoc) ⟩
    X • ((Z • Z ^ p-1) • S)  ≈⟨ cright (cleft (sym (^-+ Z 1 p-1))) ⟩
  ...
  SpushKM k 0 = begin
    ...
    ε • X ^ k • ε ∎
  SpushKM k (₁₊ m') = begin
    X ^ k • S ^ ₁₊ m'                       ≈⟨ cright (^-+ S 1 m') ⟩
    X ^ k • (S • S ^ m')                     ≈⟨ sym assoc ⟩
    (X ^ k • S) • S ^ m'                     ≈⟨ cleft (SpushK k) ⟩
    (S • X ^ k • Z ^ N) • S ^ m'             ≈⟨ assoc ⟩
    S • (X ^ k • Z ^ N) • S ^ m'             ≈⟨ cright assoc ⟩
    S • X ^ k • (Z ^ N • S ^ m')             ≈⟨ cright (cright (comm⇒pow-comm N m' (CL.lemma-comm-Z-S n))) ⟩
    ...
    S • S ^ m' • (X ^ k • Z ^ (m' Nat.* N)) • Z ^ N
                                             ≈⟨ cright (cright assoc) ⟩
    S • S ^ m' • X ^ k • (Z ^ (m' Nat.* N) • Z ^ N)
                                             ≈⟨ cright (cright (cright (sym (^-+ Z (m' Nat.* N) N)))) ⟩
...
module Decomp (n : ℕ) (x : ℤ* ₚ) where
  ...
  pushZXblk d e k = begin
    ...
    (Z ^ d • S ^ k) • (H • Z ^ N • X ^ F')           ≈⟨ cleft (comm⇒pow-comm d k (CL.lemma-comm-Z-S n)) ⟩
  ...
  pushZXtail d e k W = begin
    ...
    (Z ^ d • S ^ k) • (H • Z ^ N • X ^ F' • W) ≈⟨ cleft (comm⇒pow-comm d k (CL.lemma-comm-Z-S n)) ⟩
  ...
  M-decomp = begin
    ...
    S ^ a • H • S ^ b • H • Z ^ (p-1 Nat.* (a Nat.* z)) • X ^ (b Nat.* (p-1 Nat.* (a Nat.* z))) • X ^ (b Nat.* z) • (S ^ a • (H • X ^ (a Nat.* z)))
      ≈⟨ cright (cright (cright (cright (cright (trans (sym assoc) (cleft (sym (^-+ X (b Nat.* (p-1 Nat.* (a Nat.* z))) (b Nat.* z))))))))) ⟩
    ...
    S ^ a • H • S ^ b • H • S ^ a • H • Z ^ (p-1 Nat.* E) • X ^ F • X ^ (a Nat.* z)
      ≈⟨ cright (cright (cright (cright (cright (cright (cright (sym (^-+ X F (a Nat.* z)))))))) ) ⟩
...
-- ════════════════════════════════════════════════════════════════════
open import Examples.Groups.Symplectic.Clifford.Clifford-Lemmas p-3 p-prime g* g-gen using (lemma-comm-Z↑-CZ ; lemma-comm-Z-CZ ; lemma-M₋₁-CZ)
...
module SemiCZ (n : ℕ) where
  ...
  -- lift M-decomp-clean and regroup multiplier · Pauli
  decomp↑ : (M g*) ↑ ≈ Wg↑ • P↑
  ...
  -- (Z↑)^k commutes with CZ
  commZ↑CZ : (Z ↑) ^ zZ • CZ ≈ CZ • (Z ↑) ^ zZ
  commZ↑CZ = comm⇒pow-comm zZ 1 lemma-comm-Z↑-CZ

  -- Mg↑ commutes with Z↓ (different qudits)
  commMgZ : (M g*) ↑ • (Z ↓) ^ zX ≈ (Z ↓) ^ zX • (M g*) ↑
  commMgZ = sym (comm⇒pow-comm zX 1 (lemma-comm-Z-w↑ (M g*)))

  -- push the whole Pauli P↑ through CZ
  ...
  simplified-semi-M↑CZ : Wg↑ • CZ ≈ (Z ↓) ^ zX • CZ^ g • Wg↑
  simplified-semi-M↑CZ = •-cancelʳ {h = P↑} (begin
    (Wg↑ • CZ) • P↑                       ≈⟨ assoc ⟩
    ...
    ((Z ↓) ^ zX • (M g*) ↑) • CZ          ≈⟨ assoc ⟩
    (Z ↓) ^ zX • ((M g*) ↑ • CZ)          ≈⟨ cright (axiom semi-M↑CZ) ⟩
  ...
  commZ↓CZ = comm⇒pow-comm zX (toℕ g) lemma-comm-Z-CZ
...
module SemiCZ↓ (n : ℕ) where
  ...
  decompWP : M g* ≈ Wg • P
  decompWP = trans D2.M-decomp-clean (by-passoc (□ ^ 8) (□ ^ 6 • □ ^ 2) auto)

  relpow↓ : ∀ k → CZ • X ^ k ≈ X ^ k • (Z ↑) ^ k • CZ
  relpow↓ k = trans (lemma-Induction (trans (axiom rel-X↓-CZ) (sym assoc)) k)
                    (trans (cleft (^-• (X ↓) (Z ↑) k (lemma-comm-X-w↑ Z))) assoc)

  -- per-factor commutes with Q = (Z↑)^zX
  commSᵏQ : ∀ k → S ^ k • Q ≈ Q • S ^ k
  commSᵏQ k = comm⇒pow-comm k zX (lemma-comm-S-w↑ Z)
  commHQ : H • Q ≈ Q • H
  commHQ = comm⇒pow-comm 1 zX (lemma-comm-H-w↑ Z)
  commZQ : Z ^ zZ • Q ≈ Q • Z ^ zZ
  commZQ = comm⇒pow-comm zZ zX (lemma-comm-Z-w↑ Z)
  commXQ : X ^ zX • Q ≈ Q • X ^ zX
  commXQ = comm⇒pow-comm zX zX (lemma-comm-X-w↑ Z)
  commZCZ : Z ^ zZ • CZ ≈ CZ • Z ^ zZ
  commZCZ = comm⇒pow-comm zZ 1 lemma-comm-Z-CZ

  -- Wg commutes with Q (chain Q leftward past the 6 factors)
  commWgZ↑ : Wg • Q ≈ Q • Wg
  commWgZ↑ = trans (by-passoc (□ ^ 6 • □) (□ ^ 7) auto)
  ...
  simplified-semi-M↓CZ : Wg • CZ ≈ Q • CZ^ g • Wg
  simplified-semi-M↓CZ = •-cancelʳ {h = P} (begin
    (Wg • CZ) • P                     ≈⟨ assoc ⟩
    Wg • (CZ • P)                     ≈⟨ cright CZP↓push ⟩
    Wg • (P • (Q • CZ))               ≈⟨ sym assoc ⟩
    ...
    (Q • M g*) • CZ                   ≈⟨ assoc ⟩
    Q • (M g* • CZ)                   ≈⟨ cright (axiom semi-M↓CZ) ⟩
...
-- It is the same derivation as SemiCZ↓ but at x = -1 (so Wg = W₋₁ = (S⁻¹H)³,
-- the bare-S,H multiplier of M₋₁ = H²), using the original M₋₁·CZ relation
-- (lemma-M₋₁-CZ) in place of the generator's `axiom semi-M↓CZ`.  Concretely
-- a, b, zX all become p-1, so it reads
--   (S⁻¹H)³ · CZ  ≈  (Z↑)⁻¹ · CZ⁻¹ · (S⁻¹H)³ .
-- ════════════════════════════════════════════════════════════════════
module SemiCZ↓₋₁ (n : ℕ) where
  open PB ((₂₊ n) QRel,_===_)
  ...
  -- (S⁻¹H)³ · CZ  ≈  (Z↑)⁻¹ · CZ⁻¹ · (S⁻¹H)³
  simplified-semi-M↓CZ₋₁ : Wg • CZ ≈ Q • CZ^ (-'₁ .proj₁) • Wg
  simplified-semi-M↓CZ₋₁ = •-cancelʳ {h = P} (begin
    (Wg • CZ) • P                          ≈⟨ assoc ⟩
    Wg • (CZ • P)                          ≈⟨ cright CZP↓push ⟩
    Wg • (P • (Q • CZ))                    ≈⟨ sym assoc ⟩
    (Wg • P) • (Q • CZ)                    ≈⟨ cleft (sym decompWP) ⟩
    M -'₁ • (Q • CZ)                       ≈⟨ sym assoc ⟩
    (M -'₁ • Q) • CZ                       ≈⟨ cleft commMgZ↑ ⟩
    (Q • M -'₁) • CZ                       ≈⟨ assoc ⟩
    Q • (M -'₁ • CZ)                       ≈⟨ cright lemma-M₋₁-CZ ⟩
    Q • (CZ^ (-'₁ .proj₁) • M -'₁)         ≈⟨ cright (cright decompWP) ⟩
    Q • (CZ^ (-'₁ .proj₁) • (Wg • P))      ≈⟨ trans (cright (sym assoc)) (sym assoc) ⟩
    (Q • CZ^ (-'₁ .proj₁) • Wg) • P ∎)
...
module SemiS (n : ℕ) where
  ...
  Pstep = begin
    ...
    S • ((Z ^ zZ • Z ^ (p-1 Nat.* zX Nat.+ z½)) • X ^ zX)     ≈⟨ cright (cleft (sym (^-+ Z zZ (p-1 Nat.* zX Nat.+ z½)))) ⟩
    S • (Z ^ (zZ Nat.+ (p-1 Nat.* zX Nat.+ z½)) • X ^ zX)     ≈⟨ cright (cleft (refl' (Eq.cong (Z ^_) arith2))) ⟩
    S • (Z ^ (z½ Nat.+ E') • X ^ zX)                          ≈⟨ cright (cleft (^-+ Z z½ E')) ⟩
  ...
  Zpzx : Z ^ (p Nat.* zX) ≈ ε
  Zpzx = trans (sym (^^ Z p zX)) (trans (^-cong (Z ^ p) ε zX (CL.lemma-order-Z n)) (ε^k=ε zX))
  Zred : Z ^ (zX Nat.+ E') ≈ Z ^ zZ
  Zred = trans (refl' (Eq.cong (Z ^_) arith3)) (trans (^-+ Z zZ (p Nat.* zX)) (trans (cright Zpzx) right-unit))
    where
  ...
  -- THE simplified relation: Wg·𝑠 ≈ 𝑠^(g²)·Wg·Z^(½(g-1))
  simplified-semi-M𝑠 : Wg • 𝑠 ≈ 𝑠^ (g * g) • Wg • Z ^ zX
  simplified-semi-M𝑠 = •-cancelʳ {h = Z ^ E' • X ^ zX} (trans lhs (sym rhs))
    where
    lhs : (Wg • 𝑠) • (Z ^ E' • X ^ zX) ≈ 𝑠^ (g * g) • (Wg • P)
    lhs = begin
      (Wg • 𝑠) • (Z ^ E' • X ^ zX)   ≈⟨ assoc ⟩
      Wg • (𝑠 • (Z ^ E' • X ^ zX))   ≈⟨ cright (sym Pstep) ⟩
      Wg • (P • 𝑠)                    ≈⟨ sym assoc ⟩
      (Wg • P) • 𝑠                    ≈⟨ cleft (sym decompWP) ⟩
      M g* • 𝑠                        ≈⟨ axiom semi-M𝑠 ⟩
      𝑠^ (g * g) • M g*               ≈⟨ cright decompWP ⟩
      𝑠^ (g * g) • (Wg • P) ∎
    ...
    rhs = begin
      ...
      𝑠^ (g * g) • (Wg • ((Z ^ zX • Z ^ E') • X ^ zX))      ≈⟨ cright (cright (cleft (sym (^-+ Z zX E')))) ⟩
      𝑠^ (g * g) • (Wg • (Z ^ (zX Nat.+ E') • X ^ zX))      ≈⟨ cright (cright (cleft Zred)) ⟩
      𝑠^ (g * g) • (Wg • (Z ^ zZ • X ^ zX)) ∎

  -- 𝑠 = S·Z^½ still hides a Pauli.  Expanding both 𝑠's via 𝑠-collect pulls the
  -- Z^½ Paulis out, giving the relation in basic gates S, H, Z (no 𝑠 left):
  --   (S^a·H·S^b·H·S^a·H)·S·Z^½ ≈ S^(g²)·Z^(g²·½)·(S^a·H·S^b·H·S^a·H)·Z^(½(g-1))
  pushed-semi-M𝑠 : Wg • S • Z ^ z½
                 ≈ S ^ toℕ (g * g) • Z ^ (toℕ (g * g) Nat.* z½) • Wg • Z ^ zX
  pushed-semi-M𝑠 = begin
    Wg • S • Z ^ z½                                                ≈⟨ simplified-semi-M𝑠 ⟩
    𝑠^ (g * g) • Wg • Z ^ zX                                       ≈⟨ cleft (𝑠-collect (toℕ (g * g))) ⟩
    (S ^ toℕ (g * g) • Z ^ (toℕ (g * g) Nat.* z½)) • (Wg • Z ^ zX) ≈⟨ assoc ⟩
    S ^ toℕ (g * g) • Z ^ (toℕ (g * g) Nat.* z½) • Wg • Z ^ zX ∎

-- ════════════════════════════════════════════════════════════════════
-- Push a Pauli Z^m through Wg = S^a·H·S^b·H·S^a·H.  Wg is the multiplier
-- of M(g) (a diagonal symplectic diag(g,g⁻¹)), so it conjugates Z to a pure
-- Z power: the X-part produced along the way vanishes because a·b = g·g⁻¹ = 1.
-- ════════════════════════════════════════════════════════════════════
module SemiS-collected (n : ℕ) where
  open PB ((₁₊ n) QRel,_===_)
  open PP ((₁₊ n) QRel,_===_)
  open SR word-setoid
  open Pattern-Assoc
  ...
  -- RHS written fully right-associated (definitionally = Wg · Z^… · X^…).
  Z-Wg-cascade : ∀ m →
  ...
  Z-Wg-cascade m = begin
    ...
    Z ^ m • (S ^ a • (H • (S ^ b • (H • (S ^ a • H)))))
      ≈⟨ sym assoc ⟩
    (Z ^ m • S ^ a) • (H • (S ^ b • (H • (S ^ a • H))))
      ≈⟨ cleft (comm⇒pow-comm m a (CL.lemma-comm-Z-S n)) ⟩
  ...
  -- RHS keeps the S,H block explicit (= Wg definitionally) so by-passoc can
  -- regroup at the atom level (Wg's symbolic S-powers make `by-assoc` stuck).
  ...
  collected-semi-M𝑠 : Wg • S • Z ^ z½ ≈ S ^ c • Wg • Z ^ (toℕ (g * 1/2) Nat.+ zX)
  collected-semi-M𝑠 = begin
    Wg • S • Z ^ z½                              ≈⟨ SemiS.simplified-semi-M𝑠 n ⟩
    ...
    S ^ c • (Wg • (Z ^ toℕ (g * 1/2) • Z ^ zX))  ≈⟨ cright (cright (sym (^-+ Z (toℕ (g * 1/2)) zX))) ⟩
  ...
  final-semi-M𝑠 = •-cancelʳ {h = Z ^ z½} (begin
    (Wg • S) • Z ^ z½                              ≈⟨ assoc ⟩
...
module MgPowS (n : ℕ) where
  ...
  commZP e = begin
    ...
    ZZcomm = trans (sym (^-+ Z e zZ))
             (trans (refl' (Eq.cong (Z ^_) (NP.+-comm e zZ))) (^-+ Z zZ e))
  ...
  lemma-Mg^kS k = •-cancelʳ {h = Z ^ z½} (begin
    ...
    splitZ = begin
      ...
      Z ^ (toℕ ((g ^′ k + (- 1ₚ)) * 1/2) Nat.+ z½)                 ≈⟨ ^-+ Z (toℕ ((g ^′ k + (- 1ₚ)) * 1/2)) z½ ⟩
