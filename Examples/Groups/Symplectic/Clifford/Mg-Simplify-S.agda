...
--
-- This is the Simplified-presentation twin of Examples.Groups.Symplectic.Clifford.Mg-Simplify.
...
module Examples.Groups.Symplectic.Clifford.Mg-Simplify-S
  (p-3 : ℕ)
  (let p-2 = ₁₊ p-3)
...
open import Examples.Groups.Symplectic.Clifford.Clifford-Mod-Scalars-Simplified p-3 p-prime g* g-gen
open Simplified-Relations
open import Examples.Groups.Symplectic.Clifford.Simplified-Lemmas p-3 p-prime g* g-gen
...
module SemiCZ (n : ℕ) where
  ...
  open SR word-setoid
  open Pattern-Assoc
  open Group-Lemmas ((₂₊ n) QRel,_===_) (grouplike {₂₊ n}) using (•-cancelˡ)
  module D = Decomp n g*
  private
    a  = toℕ (g* .proj₁)
    b  = toℕ ((g* ⁻¹) .proj₁)
  ...
  completeness-semi-M↑CZ = •-cancelˡ (begin
...
module SemiS-rev (n : ℕ) where
  ...
  lemma-WgS^k (₂₊ k') = begin
    ...
    S ^ c • (S ^ (₁₊ k' Nat.* c) • (Wg • (Z ^ (₁₊ k' Nat.* d) • Z ^ d))) ≈⟨ cright (cright (cright (sym (^-+ Z (₁₊ k' Nat.* d) d)))) ⟩
    S ^ c • (S ^ (₁₊ k' Nat.* c) • (Wg • Z ^ (₁₊ k' Nat.* d Nat.+ d)))   ≈⟨ sym assoc ⟩
    (S ^ c • S ^ (₁₊ k' Nat.* c)) • (Wg • Z ^ (₁₊ k' Nat.* d Nat.+ d))   ≈⟨ cleft (sym (^-+ S c (₁₊ k' Nat.* c))) ⟩
