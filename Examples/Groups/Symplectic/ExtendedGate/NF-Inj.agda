...
lemma-act-nf {₁₊ n} (ih , lm) ps =
  ...
  begin
    act [ (ih , lm) ] ps                          ≡⟨ auto ⟩
    act ([ ih ] ↑) (act [ lm ]ᵐˡ ps)             ≡⟨ Eq.cong (act ([ ih ] ↑)) (Eq.sym (lemma-aux-vec n s)) ⟩
...
--   Write G_i = act [lm_i]ᵐˡ and M_i = act-nf ih_i.  The hypothesis expands to
--     head(G_i ps) ∷ M_i(tail(G_i ps)) equal for i=1,2 at every ps.
--
--   Step 1 – lm₁ = lm₂: head(act-nf (ih,lm) ps) = head(act [lm]ᵐˡ ps) by computation,
...
--   M₁(tail(G ps)) = M₂(tail(G ps)) for all ps (where G = act [lm₁]ᵐˡ).
...
lemma-nf-inj {₁₊ n} (ih₁ , lm₁) (ih₂ , lm₂) hyp = ≡×≡⇒≡ (ih-eq , lm-eq)
  ...
  head-eq : ∀ ps → head (act [ lm₁ ]ᵐˡ ps) ≡ head (act [ lm₂ ]ᵐˡ ps)
  ...
  tail-agree : ∀ ps → act-nf ih₁ (tail (act [ lm₁ ]ᵐˡ ps))
                     ≡ act-nf ih₂ (tail (act [ lm₁ ]ᵐˡ ps))
  tail-agree ps = Eq.cong tail
    (subst (λ lm' → head (act [ lm₁ ]ᵐˡ ps) ∷ act-nf ih₁ (tail (act [ lm₁ ]ᵐˡ ps))
