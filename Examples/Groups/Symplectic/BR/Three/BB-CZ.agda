...
module Examples.Groups.Symplectic.BR.Three.BB-CZ (p-2 : ℕ) (p-prime : Prime (2+ p-2))  where
...
open import Examples.Groups.Symplectic.Embeding-2n p-2 p-prime 1 renaming (f* to emb ; by-emb' to lemma-cong⇣' ; by-emb to lemma-cong⇣ )
...
aux-BB~DD b d = begin
  ...
  H ↑ ↑ • (Ex ↑ • CZ^ (- d) ↑) • Ex • CZ^ (- b) • H ^ 3 ≈⟨ cright cleft lemma-cong↑ _ _ (B2.sym (P2.comm⇒pow-comm (toℕ (- d)) 1 lemma-comm-Ex-CZ-n)) ⟩
  ...
  (H ↑ ↑ • Ex • Ex ↑) • CZ^ (- d) • (CZ^ (- b) • Ex) ↑ • Ex • Ex ↑ • Ex • H ^ 3 ≈⟨ cong (sym (by-ex {w = ((EX.H EX.↑ EX.↑ • EX.Ex • EX.Ex EX.↑ • EX.Ex) • EX.Ex)} {v = EX.H EX.↑ EX.↑ • EX.Ex • EX.Ex EX.↑} (rewrite-EX 100 auto)) ) (cright (cleft lemma-cong↑ _ _ (P2.comm⇒pow-comm (toℕ (- b)) 1 lemma-comm-Ex-CZ-n))) ⟩
