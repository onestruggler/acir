...
module Examples.Groups.Symplectic.BR.Three.Lemmas2 (p-2 : ℕ) (p-prime : Prime (2+ p-2))  where
...
aux-CZ-H↑-CZ↑^k k = bbc (Ex ↑) (Ex ↑) claim
  ...
  claim = begin
    ...
    (Ex ↑ • CZ • H ↑) • CZ^ k ↑ • Ex ↑ ≈⟨ cright lemma-cong↑ _ _ (P2.comm⇒pow-comm (toℕ k) 1 lemma-comm-Ex-CZ-n) ⟩
...
aux-comm-CZ02-CZ02^ k = begin
  ...
  (Ex • CZ ↑ • Ex • Ex) • CZ^ k ↑ • Ex ≈⟨ cleft rewrite-swap 100 auto ⟩
  (Ex • CZ ↑) • CZ^ k ↑ • Ex ≈⟨ sa (□ ^ 2 • □ ^ 2) (□ • □ ^ 2 • □) auto ⟩
  Ex • (CZ ↑ • CZ^ k ↑) • Ex ≈⟨ cright cleft lemma-cong↑ _ _ (P2.comm⇒pow-comm 1 (toℕ k) B2.refl) ⟩
...
lemma-|||-mmh m↑@(m↑' , nz↑) k  m@(m' , nz) = begin
  CZ02 • ⟦ m↑ ⟧ₘ ↑ ↑ • CZ • ⟦ m , HS^ k ⟧ₘ₊ ↑ • CZ ↑ ≈⟨ cright cright cright by-passoc (□ ^ 3 • □) (□ ^ 4) auto ⟩
  CZ02 • ⟦ m↑ ⟧ₘ ↑ ↑ • CZ • ⟦ m ⟧ₘ ↑ • H ↑ • S^ k ↑ • CZ ↑ ≈⟨ cright cright cright cright (cright lemma-cong↑ _ _ (P2.comm⇒pow-comm (toℕ k) 1 (B2.sym (B2.axiom comm-CZ-S↓)))) ⟩
  ...
  (CZ02 • CX ↑ ^ toℕ (- m↑/m)) • CZ02^ (- m↑/m) • CZ • (H ↑ • ⟦ m↑ ⟧ₘ ↑ ↑) • ⟦ m ⁻¹ ⟧ₘ ↑ • S^ k ↑ ≈⟨ cong (comm⇒pow-comm 1 (toℕ (- m↑/m)) lemma-comm-CZ02-CX) (cright cright cleft lemma-cong↑ _ _ (lemma-comm-H-w↑ ⟦ m↑ ⟧ₘ)) ⟩
...
aux-||hh| k*@(k , nz) = begin
  ...
  (CZ02 • H ↑ ↑) • (CX ↑ ^ toℕ (- k) • CZ02'^ (- k)) • CZ • H ↑ ≈⟨ cright cleft comm⇒pow-comm (toℕ (- k)) (toℕ (- k)) (sym lemma-comm-CZ02-CX) ⟩
...
lemma-|||-mhmh m↑@(m↑' , nz↑) k  m@(m' , nz) l = begin
  ...
  CZ02 • ⟦ m↑ , HS^ k ⟧ₘ₊ ↑ ↑ • CZ • ⟦ m ⟧ₘ ↑ • H ↑ • S^ l ↑ • CZ ↑ ≈⟨ cright cright cright cright (cright lemma-cong↑ _ _ (P2.comm⇒pow-comm (toℕ l) 1 (B2.sym (B2.axiom comm-CZ-S↓)))) ⟩
