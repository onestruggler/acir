...
module Examples.Groups.Symplectic.BR.Three.DD-CZ (p-2 : ℕ) (p-prime : Prime (2+ p-2))  where
...
open import Examples.Groups.Symplectic.Ex-Sym4 p-2 p-prime hiding (lemma-Ex-S^ᵏ ; lemma-Ex-S^ᵏ↑)
...
aux-Ex-D↑ b@(₁₊ _) nz = begin
  ...
  Ex • Ex ↑ • CZ ↑ ^ toℕ (- b) • CZ ≈⟨ cright cright comm⇒pow-comm (toℕ (- b)) 1 (axiom selinger-c12) ⟩
...
aux-D-Ex↑-CZ^k k b@(₁₊ _) nz = begin
  ...
  (CZ^ (- b) • CZ ↑ ^ toℕ k) • (Ex • Ex ↑) ≈⟨ cleft comm⇒pow-comm (toℕ (- b)) (toℕ k) (sym (axiom selinger-c12)) ⟩
...
lemma-dir-and-vd' vd@((₀ , b) ∷ (₀ , d) ∷ []) = begin
  ...
  (Ex • CZ^ (- b)) • Ex ↑ • CZ^ (- d) ↑ • CZ ≈⟨ cong (comm⇒pow-comm 1 (toℕ (- b)) (sym lemma-comm-Ex-CZ-n)) (cright sym (aux-comm-CZ-CZ^k↑ (- d))) ⟩
  (CZ^ (- b) • Ex) • Ex ↑ • CZ • CZ^ (- d) ↑ ≈⟨ sa (□ ^ 2 • □ ^ 3) (□ • □ ^ 3 • □) auto ⟩
  CZ^ (- b) • (Ex • Ex ↑ • CZ) • CZ^ (- d) ↑ ≈⟨ cright cleft lemma-Ex-Ex↑-CZ ⟩
  CZ^ (- b) • (CZ ↑ • Ex • Ex ↑) • CZ^ (- d) ↑ ≈⟨ sa (□ • □ ^ 3 • □) (□ ^ 2 • □ ^ 3) auto ⟩
  (CZ^ (- b) • CZ ↑) • Ex • Ex ↑ • CZ^ (- d) ↑ ≈⟨ cleft comm⇒pow-comm (toℕ (- b)) 1 (sym (axiom selinger-c12)) ⟩
...
lemma-dir-and-vd' vd@(d1@(₀ , b) ∷ d2@(c@(₁₊ _) , d) ∷ []) = begin
  ...
  H ↑ ↑ • (CZ^ (- b) • CZ ↑) • Ex • Ex ↑ • CZ02^ c • CX^ (- c) ↑ • S^ -d/c ↑ ≈⟨ cright cleft comm⇒pow-comm (toℕ (- b)) 1 (sym (axiom selinger-c12)) ⟩
  ...
  (H ↑ ↑ • CZ ↑) • Ex • (CZ^ (- b + c) • H ↑ ↑ ^ 3) • Ex ↑ • CZ^ (- c) ↑ • H ↑ • S^ -d/c ↑ ≈⟨ cright cright cleft comm⇒pow-comm (toℕ (- b + c)) 3 (lemma-comm-CZ-w↑↑ H) ⟩
...
lemma-dir-and-vd' vd@(d1@(a1@(₁₊ _) , b1) ∷ d2@(a2@(₁₊ _) , b2) ∷ []) = begin
  ...
  (Ex • Ex ↑) • (CZ02^ (- a1) • H • CZ^ (- a2) ↑ • H ↑) • (S^ -b1/a1 • CZ) • S^ -b2/a2 ↑ ≈⟨ cright cright cleft comm⇒pow-comm (toℕ -b1/a1) 1 (sym (axiom comm-CZ-S↓)) ⟩
