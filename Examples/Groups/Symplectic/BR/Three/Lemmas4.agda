...
module Examples.Groups.Symplectic.BR.Three.Lemmas4 (p-2 : ℕ) (p-prime : Prime (2+ p-2))  where
...
open import Examples.Groups.Symplectic.Ex-Sym4 p-2 p-prime hiding (lemma-Ex-S^ᵏ↑)
...
open import Examples.Groups.Symplectic.Ex-Sym4n3 p-2 p-prime
open import Examples.Groups.Symplectic.Ex-Sym4n4 p-2 p-prime
...
open import Examples.Groups.Symplectic.Lemmas4-Sym p-2 p-prime as L4
...
open import Examples.Groups.Symplectic.XEX-Rewriting p-2 p-prime
open Rewriting-EX 2
...
open import Examples.Groups.Symplectic.Embeding-2n p-2 p-prime 1
open import Examples.Groups.Symplectic.XEX-Rewriting p-2 p-prime
...
lemma-CX02-CZ^k k = bbc Ex ε claim
  ...
  claim = begin
    ...
    (CX ↑ • Ex) • CZ^ k ≈⟨ assoc ⟩
    CX ↑ • Ex • CZ^ k ≈⟨ cright comm⇒pow-comm 1 (toℕ k) (sym lemma-comm-Ex-CZ-n) ⟩
    ...
    (CZ^ k • Ex) • CZ^ (- k) ↑ • CX02 ≈⟨ cleft comm⇒pow-comm (toℕ k) 1 lemma-comm-Ex-CZ-n ⟩
...
lemma-CX02-CX↑-CZ^k-alt k = begin
  ...
  CZ^ k • S^ (- k) • (CZ ↑ ^ toℕ (- k) • CX02) • S^ k • S^ k ↑ ↑ • CX ↑ ≈⟨ cright cright cleft comm⇒pow-comm (toℕ (- k)) 1 (sym aux-comm-CX02-CZ↑) ⟩
  CZ^ k • S^ (- k) • (CX02 • CZ ↑ ^ toℕ (- k)) • S^ k • S^ k ↑ ↑ • CX ↑ ≈⟨ cright cright sa (□ ^ 2 • □ ^ 2) (□ • □ ^ 2 • □ ) auto ⟩
  CZ^ k • S^ (- k) • CX02 • (CZ ↑ ^ toℕ (- k) • S^ k) • S^ k ↑ ↑ • CX ↑ ≈⟨ cright cright cright cleft comm⇒pow-comm (toℕ (- k)) 1 (sym (lemma-comm-Sᵏ-w↑ (toℕ k) CZ)) ⟩
  CZ^ k • S^ (- k) • CX02 • (S^ k • CZ ↑ ^ toℕ (- k)) • S^ k ↑ ↑ • CX ↑ ≈⟨ cright cright cright sa (□ ^ 2 • □ ^ 2) (□ • □ ^ 2 • □ ) auto ⟩
  CZ^ k • S^ (- k) • CX02 • S^ k • (CZ ↑ ^ toℕ (- k) • S^ k ↑ ↑) • CX ↑ ≈⟨ cright cright cright cright cleft comm⇒pow-comm (toℕ (- k)) 1 (lemma-cong↑  _ _ (aux-comm-CZ-S^k↑ k)) ⟩
...
lemma-CZ02⁻¹-CZ⁻¹↑-CZ^k k = bbc (H • H ↑) ε claim
  ...
  claim = begin
    ...
    (H ^ 3 • H ↑ ^ 3) • (H ↑ ^ 2 • CZ02) • H • CZ^ (- ₁) ↑ • H ↑ • CZ^ (- k) • H ^ 2 ≈⟨ cright cleft comm⇒pow-comm 2 1 (sym (lemma-comm-CZ02-H↑ 0 ₁)) ⟩
    ...
    H ^ 3 • (H ↑ ^ 3 • CZ02) • H • CZ ↑ • H ↑ • (CZ^ k • H ↑ ^ 2 ) • H ^ 2 ≈⟨ cright cleft comm⇒pow-comm 3 1 (sym (lemma-comm-CZ02-H↑' ₁)) ⟩
    ...
    (CZ^ k • (S^ (- k) • CX02 • S^ k) • (S^ (- k) • CX) ↑) • (S^ k ↑ • H ↑ ^ 2) • H ^ 2 ≈⟨ cright cleft lemma-cong↑ _ _ (P2.comm⇒pow-comm (toℕ k) 1 ( Sym01.rewrite-sym0 100 auto)) ⟩
    ...
    (CZ^ k • (S^ (- k) • CX02 • S^ k)) • (S^ (- k) ↑ • H ↑ ^ 2) • CX^ (- ₁) ↑ • S^ k ↑ • H ^ 2 ≈⟨ cright cong (lemma-cong↑ _ _ (P2.comm⇒pow-comm (toℕ (- k)) 1 ( Sym01.rewrite-sym0 100 auto))) refl ⟩
    ...
    (CZ^ k • S^ (- k)) • (CX02 • H ↑ ^ 2) • (S^ k • H ^ 2) • (S^ (- k) ↑ • CX^ (- ₁) ↑ • S^ k ↑) ≈⟨ cright cong (comm⇒pow-comm 1 2 (lemma-comm-CX02-H↑' ₁)) (cleft comm⇒pow-comm (toℕ (k)) 1 ( rewrite-sym0 100 auto)) ⟩
    ...
    (CZ^ k • H ↑ ^ 2) • (S^ (- k) • H ^ 2) • CX02^ (- ₁) • S^ k • (S^ (- k) ↑ • CX^ (- ₁) ↑ • S^ k ↑) ≈⟨ cong (sym (lemma-semi-HH↑-CZ^k'' k)) (cleft comm⇒pow-comm (toℕ (- k)) 1 ( rewrite-sym0 100 auto)) ⟩
...
aux-MHS↑-CZ m k l = begin
  ...
  (⟦ m ⁻¹ ⟧ₘ ↑ • H ↑) • S^ k ↑ • CZ^ l ≈⟨ cong (sym (lemma-cong↑ _ _ (semi-HM m))) (sym (comm⇒pow-comm (toℕ l) 1 (aux-comm-CZ-S^k↑ k))) ⟩
...
aux-MHS↓-CZ m k l = begin
  ⟦ m ⁻¹ , HS^ k ⟧ₘ₊ ↓ • CZ^ l ≈⟨ sa (□ ^ 3 • □) (□ ^ 2 • □ ^ 2) auto ⟩
  (⟦ m ⁻¹ ⟧ₘ ↓ • H ↓) • S^ k ↓ • CZ^ l ≈⟨ cong (sym ((L02.semi-HM m))) (sym (comm⇒pow-comm (toℕ l) 1 (comm⇒pow-comm 1 (toℕ k) (axiom comm-CZ-S↓)))) ⟩
  (H ↓ • ⟦ m ⟧ₘ ↓) • CZ^ l • S^ k ↓ ≈⟨ sa (□ ^ 2 • □ ^ 2) (□ • □ ^ 2 • □) auto ⟩
  H ↓ • (⟦ m ⟧ₘ ↓ • CZ^ l) • S^ k ↓ ≈⟨ cright cleft (lemma-M↓CZ^k (m .proj₁) l (m .proj₂)) ⟩
  H ↓ • (CZ^ (l * m .proj₁) • ⟦ m ⟧ₘ ↓) • S^ k ↓ ≈⟨ sa (□ • □ ^ 2 • □) (□ ^ 4) auto ⟩
  H ↓ • CZ^ (l * m .proj₁) • ZM m ↓ • S^ k ↓ ∎
