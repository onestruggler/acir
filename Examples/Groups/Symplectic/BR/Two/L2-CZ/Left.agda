...
--
-- L2-CZ, the L' 2 component (the j=0 boxes).
--
-- One of the two halves of the old lemma-dir-and-l', which was a case
-- split on L' 2 ⊎ L' 1; this is the inj1 half and carries essentially
-- all of the work -- nine clauses of equational reasoning against the
-- B, A and M boxes.  L2-CZ combines it with the L' 1 half.
------------------------------------------------------------------------
...
module Examples.Groups.Symplectic.BR.Two.L2-CZ.Left (p-2 : ℕ) (p-prime : Prime (2+ p-2)) where
...
l'-of₁ (((c , d) ∷ []) , ((a@₀ , b@(₁₊ _)) , _)) with b ≟ c
l'-of₁ (((c , d) ∷ []) , ((a@₀ , b@(₁₊ _)) , _)) | yes eq  = inj₂ ([] , (b , d) , λ ())
l'-of₁ (((c , d) ∷ []) , ((a@₀ , b@(₁₊ _)) , _)) | no neq  = inj₁ (((c , d + - a) ∷ []) , ((a , b + - c) , nzp))
  where
  nzp : (a , b + - c) ≢ (₀ , ₀)
  nzp eqp = (neq (b-c=0⇒b=c b c (Eq.cong proj₂ eqp)))
l'-of₁ (((c , d) ∷ []) , ((a@(₁₊ _) , b) , _))            = inj₁ (((c , d + - a) ∷ []) , ((a , b + - c) , λ ()))
l'-of₁ (((c , d) ∷ []) , ((a@₀ , b@₀) , nz))              = ⊥-elim (nz auto)

dir-of₁ : L' 2 → Word (Gen 2)

-- L' 2 inputs (j=0)
dir-of₁ (((c@₀ , d@₀) ∷ []) , ((a@₀ , b@(₁₊ _)) , nz)) with b ≟ c
dir-of₁ (((c@₀ , d@₀) ∷ []) , ((a@₀ , b@(₁₊ _)) , nz)) | yes ()
dir-of₁ (((c@₀ , d@₀) ∷ []) , ((a@₀ , b@(₁₊ _)) , nz)) | no neq      =   CZ^ b⁻¹
  where b⁻¹ = (((b , λ ())) ⁻¹) .proj₁

dir-of₁ (((c@₀ , d@(₁₊ _)) ∷ []) , ((a@₀ , b@(₁₊ _)) , nz)) with b ≟ c
dir-of₁ (((c@₀ , d@(₁₊ _)) ∷ []) , ((a@₀ , b@(₁₊ _)) , nz)) | yes ()
  where b⁻¹ = (((b , λ ())) ⁻¹) .proj₁
dir-of₁ (((c@₀ , d@(₁₊ _)) ∷ []) , ((a@₀ , b@(₁₊ _)) , nz)) | no neq =   S^ (- b⁻¹d + - b⁻¹d) • CZ^ b⁻¹
  where
  b⁻¹ = ((b , λ ()) ⁻¹) .proj₁
  b⁻¹d = b⁻¹ * d

dir-of₁ (((c@(₁₊ _) , d) ∷ []) , ((a@₀ , b@(₁₊ _)) , nz)) with b ≟ c
dir-of₁ (((c@(₁₊ _) , d) ∷ []) , ((a@₀ , b@(₁₊ _)) , nz)) | yes eq   =   (H • CZ^ b⁻¹ • H ^ 3) ⁻¹ʷ • HH
  where b⁻¹ = (((b , λ ())) ⁻¹) .proj₁
dir-of₁ (((c@(₁₊ _) , d) ∷ []) , ((a@₀ , b@(₁₊ _)) , nz)) | no neq   =   M [b-c]/b • H • CZ^ b⁻¹ • H ^ 3
  where
  b⁻¹ = ((b , λ ()) ⁻¹) .proj₁
  b-c≠0 : b + - c ≢ ₀
...
dir-of₁ (((c@₀ , d) ∷ []) , ((a@(₁₊ _) , b) , nz))                   =   ε
dir-of₁ (((c@(₁₊ _) , d) ∷ []) , ((a@(₁₊ _) , b) , nz))              =   H ^ 3 • S^ (- ac⁻¹) • H
  where ac⁻¹ = a * ((c , λ ()) ⁻¹) .proj₁


dir-of₁ (((c , d) ∷ []) , ((a@₀ , b@₀) , nzx)) = ⊥-elim (nzx auto)
...
lemma₁ (((c , d) ∷ []) , ((a@₀ , b@₀) , nzx)) = ⊥-elim (nzx auto)

lemma₁ l@(((c@(₁₊ _) , d) ∷ []) , ((a@(₁₊ _) , b) , nzx)) = begin
  intp (inj₁ l) • CZ ≈⟨ cleft cong left-unit refl ⟩
  ...
  (Ex • S^ (a⁻¹ * c) ↑) • H ↑ • (H ^ 3 • H ↑ ^ 2) • (CZ^ a • M (-' bb* ⁻¹) ↑) • (S^ (- (-a⁻¹ * c)) ↑  • H ↑) • (S^ (-bb⁻¹) ↑ • S^ -d/c ↑) • (S^ (a * c) • H ^ 2) • M a* • S^ -b/a ≈⟨ cong (lemma-Ex-S^ᵏ↑ (a⁻¹ * c)) (cright cong (rewrite-sym0 100 auto) (cong (lemma-CZ^kM↑ (((-' bb* ⁻¹) ) .proj₁) a ((-' bb* ⁻¹) .proj₂)) (cright cong (lemma-cong↑ _ _ (lemma-S^k+l -bb⁻¹ -d/c)) (cleft comm⇒pow-comm (toℕ (a * c)) 1 (sym (trans assoc (axiom comm-HHS))))))) ⟩
  ...
  ((S^ (- ((-' (a* *' c* ⁻¹))⁻¹) .proj₁) • H ^ 2) • H • S^ (- - ac⁻¹) • M (-' -' (a* *' c* ⁻¹))) • (Ex • CX'^ c • H ↑ • S^ (- (d + - a) * c⁻¹) ↑) • M (a* ⁻¹) • H • S^ (- (b + - c) * a⁻¹) ≈⟨ cleft cleft comm⇒pow-comm (toℕ (- ((-' (a* *' c* ⁻¹))⁻¹) .proj₁)) 1 (sym (trans assoc (axiom comm-HHS))) ⟩
...
lemma₁ l@(((c@₀ , d@(₁₊ _)) ∷ []) , ((a@(₁₊ _) , b@(₁₊ _)) , nzx)) =  begin
  intp (inj₁ l) • CZ ≈⟨  cleft cong left-unit refl ⟩
  ([ c , d ]ᵇ • XM a* • H • S^ -b/a) • CZ
    ≡⟨ Eq.cong (\ z → ([ c , d ]ᵇ • z • H • S^ -b/a) • CZ) (XM≡ZM⁻¹ a*) ⟩
  ([ c , d ]ᵇ • M a*⁻¹ • H • S^ -b/a) • CZ ≈⟨ sa (□ ^ 4 • □) (□ • □ ^ 2 • □ ^ 2) auto ⟩
...
lemma₁ l@(((c@₀ , d@₀) ∷ []) , ((a@₀ , b@(₁₊ _)) , nzx)) with b ≟ c
... | yes ()
... | no neq = begin
  intp (inj₁ l) • CZ ≈⟨  cleft cong left-unit (refl' (XM≡ZM⁻¹ (b , λ ()))) ⟩
  ([ c , d ]ᵇ • M b*⁻¹) • CZ ≈⟨ assoc ⟩
  [ c , d ]ᵇ • M b*⁻¹ • CZ ≈⟨ cright axiom (semi-M↓CZ b*⁻¹) ⟩
  [ c , d ]ᵇ • CZ^ b⁻¹ • M b*⁻¹ ≈⟨ cleft rewrite-sym0 100 auto ⟩
  Ex • CZ^ b⁻¹ • M b*⁻¹ ≈⟨ sym assoc ⟩
  (Ex • CZ^ b⁻¹) • M b*⁻¹ ≈⟨ cleft comm⇒pow-comm 1 (toℕ b⁻¹) (sym lemma-comm-Ex-CZ-n) ⟩
  (CZ^ b⁻¹ • Ex) • M b*⁻¹ ≈⟨ assoc ⟩
  CZ^ b⁻¹ • Ex • M b*⁻¹ ≈⟨ refl ⟩
  ...
  l' : L' 2 ⊎ L' 1
  l' = inj₁ (((c , d + - a) ∷ []) , ((a , b + - c) , nzp))
  dir = CZ^ b⁻¹


lemma₁ l@(((c@₀ , d@(₁₊ _)) ∷ []) , ((a@₀ , b@(₁₊ _)) , nzx)) with b ≟ c
... | yes ()
... | no neq = begin
  intp (inj₁ l) • CZ ≈⟨  cleft cong left-unit (refl' (XM≡ZM⁻¹ (b , λ ()))) ⟩
  ([ c , d ]ᵇ • M b*⁻¹) • CZ ≈⟨ assoc ⟩
  [ c , d ]ᵇ • M b*⁻¹ • CZ ≈⟨ cright axiom (semi-M↓CZ b*⁻¹) ⟩
  [ c , d ]ᵇ • CZ^ b⁻¹ • M b*⁻¹ ≈⟨ cleft rewrite-sym0 100 auto ⟩
  (Ex • CX'^ d) • CZ^ b⁻¹ • M b*⁻¹ ≈⟨ by-passoc (□ ^ 2 • □ ^ 2) (□ • □ ^ 2 • □) auto ⟩
  ...
  S^ (- b⁻¹d + - b⁻¹d) • (Ex • CZ^ b⁻¹) • CX'^ d • M b*⁻¹ ≈⟨ cright cleft comm⇒pow-comm 1 (toℕ b⁻¹) (sym lemma-comm-Ex-CZ-n) ⟩
...
lemma₁ l@(((c@(₁₊ _) , d) ∷ []) , ((a@₀ , b@(₁₊ _)) , nzx)) with b ≟ c
... | yes eq  = bbc dir-acz (CZ^ (- ₁)) claim 
  ...
  aux-dir-acz' = begin
    ...
    (dir-acz • dir-acz ⁻¹ʷ) • HH ≈⟨ cleft inverseʳ ⟩
    ε • HH ≈⟨ left-unit ⟩
    HH ∎

  claim : dir-acz • (intp (inj₁ l) • CZ) • CZ^ (- ₁) ≈ dir-acz • (dir • intp l') • CZ^ (- ₁)
  claim = begin
    dir-acz • (intp (inj₁ l) • CZ) • CZ^ (- ₁) ≈⟨ by-passoc (□ • □ ^ 2 • □) (□ ^ 4) auto ⟩
    dir-acz • intp (inj₁ l) • CZ • CZ^ (- ₁) ≈⟨ cright cright trans (lemma-CZ^k+l ₁ (- ₁)) (refl' (Eq.cong CZ^ (+-inverseʳ ₁))) ⟩
    dir-acz • intp (inj₁ l) • ε ≈⟨ cright right-unit ⟩
    dir-acz • intp (inj₁ l) ≈⟨ cright trans assoc left-unit ⟩
    ...
    dir-acz • [ b , d ]ᵇ • [ ((₀ , - b) , nz-bp) ]ᵃ • HH ≈⟨ by-passoc (□ ^ 4) (□ ^ 3 • □) auto ⟩
    ...
    HH • intp l' • CZ^ (- ₁) ≈⟨ cleft sym aux-dir-acz' ⟩
    (dir-acz • dir) • intp l' • CZ^ (- ₁) ≈⟨ by-passoc (□ ^ 2 • □ ^ 2) (□ • □ ^ 2 • □) auto ⟩
    dir-acz • (dir • intp l') • CZ^ (- ₁) ∎

... | no neq = begin
  intp (inj₁ l) • CZ ≈⟨ cleft cong left-unit (refl' (XM≡ZM⁻¹ b*)) ⟩
  ([ c , d ]ᵇ • M (b* ⁻¹)) • CZ ≈⟨ assoc ⟩
  [ c , d ]ᵇ • M (b* ⁻¹) • CZ ≈⟨ cright axiom (semi-M↓CZ (b* ⁻¹)) ⟩
  ...
  M (₁-y) • H ^ 3 • (Ex • CZ^ (- y * c⁻¹)) • (M (c* ⁻¹) ↑ • H ↑) • CX • H ↑ • M c* ↑ • S^ -d/c ↑ • M (₁-y ⁻¹) • M (b* ⁻¹) ≈⟨ cright cright cong (comm⇒pow-comm 1 (toℕ (- y * c⁻¹)) (sym lemma-comm-Ex-CZ)) (cleft sym (lemma-cong↑ _ _ (semi-HM c*))) ⟩
