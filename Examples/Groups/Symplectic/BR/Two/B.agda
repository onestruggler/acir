...
module Examples.Groups.Symplectic.BR.Two.B (p-2 : ℕ) (p-prime : Prime (2+ p-2)) where
...
open import Examples.Groups.Symplectic.BR.Two.D p-2 p-prime hiding (n ; module L01) renaming(dir-of to  dir-of-d)
open import Examples.Groups.Symplectic.Lemmas.Embeding-1n p-2 p-prime
...
lemma-B-br d@(a@₀ , b@(₁₊ _)) g@(S-gen) neqH neqCZ = begin
  ...
  (S ↑ • S^ (b * b)) • (Ex • CZ^ (- b)) • CX^ b ≈⟨ cright cleft comm⇒pow-comm 1 (toℕ (- b)) (sym lemma-comm-Ex-CZ) ⟩
