...
module Examples.Groups.Symplectic.BR.TwoQupit (p-2 : ℕ) (p-prime : Prime (2+ p-2)) where
...
fig-28-1 b*@(b , nzb) = begin
  ...
  ([ ₀ , ₀ ]ᵇ • CZ^ b⁻¹) • [ (₀ , b) , nz ]ᵃ  ≈⟨ cleft comm⇒pow-comm 1 (toℕ b⁻¹) (sym {!!}) ⟩  
  (CZ^ b⁻¹ • [ ₀ , ₀ ]ᵇ) • [ (₀ , b) , nz ]ᵃ  ≈⟨ assoc ⟩  
  CZ^ b⁻¹ • [ ₀ , ₀ ]ᵇ • [ (₀ , b) , nz ]ᵃ  ∎
  where
  b⁻¹ = (b* ⁻¹) .proj₁
...
fig-28-2 b*@(b , nzb) d*@(d , nzd) = begin
  ...
  ((Ex • CX • ⟦ (d , nzd) ⁻¹ ⟧ₘ ↑) • CZ^ b⁻¹) • [ (₀ , b) , nz ]ᵃ  ≈⟨ by-passoc ((□ ^ 3 • □) • □) (□ ^ 2 • □ ^ 2 • □) auto ⟩
  (Ex • CX) • (⟦ (d , nzd) ⁻¹ ⟧ₘ ↑ • CZ^ b⁻¹) • [ (₀ , b) , nz ]ᵃ  ≈⟨ cright cleft lemma-M↑CZ^k d⁻¹ b⁻¹ (((d , nzd) ⁻¹) .proj₂) ⟩
  (Ex • CX) • (CZ^ (b⁻¹ * d⁻¹) • ⟦ (d , nzd) ⁻¹ ⟧ₘ ↑) • [ (₀ , b) , nz ]ᵃ  ≈⟨ by-passoc (□ ^ 2 • □ ^ 2 • □) (□ • □ ^ 2 • □ ^ 2) auto ⟩
  Ex • (CX • CZ^ (b⁻¹ * d⁻¹)) • ⟦ (d , nzd) ⁻¹ ⟧ₘ ↑ • [ (₀ , b) , nz ]ᵃ  ≈⟨ cright cleft lemma-semi-CXCZ^k k* ⟩
  Ex • (S^ -2k ↑ • CZ^ k • CX) • ⟦ (d , nzd) ⁻¹ ⟧ₘ ↑ • [ (₀ , b) , nz ]ᵃ  ≈⟨ by-passoc (□ • □ ^ 3 • □) (□ ^ 2 • □ ^ 3) auto ⟩
  ...
  (S^ -2k • Ex) • CZ^ k • CX • ⟦ (d , nzd) ⁻¹ ⟧ₘ ↑ • [ (₀ , b) , nz ]ᵃ  ≈⟨ by-passoc (□ ^ 2 • □ ^ 2) (□ • □ ^ 2 • □) auto ⟩
  S^ -2k • (Ex • CZ^ k) • CX • ⟦ (d , nzd) ⁻¹ ⟧ₘ ↑ • [ (₀ , b) , nz ]ᵃ  ≈⟨ cright cleft comm⇒pow-comm 1 (toℕ k) (sym lemma-comm-Ex-CZ-n) ⟩
  S^ -2k • (CZ^ k • Ex) • CX • ⟦ (d , nzd) ⁻¹ ⟧ₘ ↑ • [ (₀ , b) , nz ]ᵃ  ≈⟨ by-passoc (□ • □ ^ 2 • □ ^ 3) (□ ^ 2 • □ ^ 3 • □) auto ⟩
...
aux-HH↑-CZ⁻¹ = begin
  ...
  (CZ^ ₋₁) ^ p-1 • H ↑ ^ 2 ≈⟨ cleft ^-cong _ _ p-1 (refl' (Eq.cong (CZ ^_) lemma-toℕ₋₁)) ⟩
...
fig-28-3 b*@(b , nzb) d = begin
  ...
  (Ex • CX • ⟦ (b , nzb) ⁻¹ , HS^ -d/b ⟧ₘ₊ ↑) • CZ^ b⁻¹ • ⟦ (b , nzb) ⁻¹ ⟧ₘ ≈⟨ by-passoc (□ ^ 5 • □ ^ 2) (□ ^ 4 • □ ^ 2 • □) auto ⟩
  (Ex • CX • ⟦ (b , nzb) ⁻¹ ⟧ₘ ↑ • H ↑) • (S^ -d/b ↑ • CZ^ b⁻¹) • ⟦ (b , nzb) ⁻¹ ⟧ₘ ≈⟨ cright cleft sym (comm⇒pow-comm (toℕ b⁻¹) 1 (aux-comm-CZ-S^k↑ (-d/b))) ⟩
  (Ex • CX • ⟦ (b , nzb) ⁻¹ ⟧ₘ ↑ • H ↑) • (CZ^ b⁻¹ • S^ -d/b ↑) • ⟦ (b , nzb) ⁻¹ ⟧ₘ ≈⟨ sym (cleft cright cright lemma-cong↑ _ _ (semi-HM (b , nzb))) ⟩
  (Ex • CX • H ↑ • ⟦ (b , nzb) ⟧ₘ ↑) • (CZ^ b⁻¹ • S^ -d/b ↑) • ⟦ (b , nzb) ⁻¹ ⟧ₘ ≈⟨ by-passoc (□ ^ 4 • □ ^ 2 • □) (□ ^ 3 • □ ^ 2 • □ ^ 2) auto ⟩
  ...
  (H ^ 3 • CZ • H ^ 3 • H ↑) • ⟦ (b , nzb) ⟧ₘ ↑ • S^ -d/b ↑ • ⟦ (b , nzb) ⁻¹ ⟧ₘ ≈⟨ by-passoc (□ ^ 4 • □ ^ 3) (□ ^ 3 • □ ^ 2 • □ ^ 2) auto ⟩
  (H ^ 3 • CZ • H ^ 3) • (H ↑ • ⟦ (b , nzb) ⟧ₘ ↑) • S^ -d/b ↑ • ⟦ (b , nzb) ⁻¹ ⟧ₘ ≈⟨ cright cleft lemma-cong↑ _ _ (semi-HM (b , nzb)) ⟩
  (H ^ 3 • CZ • H ^ 3) • (⟦ (b , nzb) ⁻¹ ⟧ₘ ↑ • H ↑) • S^ -d/b ↑ • ⟦ (b , nzb) ⁻¹ ⟧ₘ ≈⟨ cright by-passoc (□ ^ 2 • □ ^ 2) (□ ^ 3 • □) auto ⟩
  (H ^ 3 • CZ • H ^ 3) • (⟦ (b , nzb) ⁻¹ ⟧ₘ ↑ • H ↑ • S^ -d/b ↑) • ⟦ (b , nzb) ⁻¹ ⟧ₘ ≈⟨ cright sym (aux-comm-MMC ((b , nzb) ⁻¹) ((b , nzb) ⁻¹) (HS^ -d/b)) ⟩
  (H ^ 3 • CZ • H ^ 3) • ⟦ (b , nzb) ⁻¹ ⟧ₘ • (⟦ (b , nzb) ⁻¹ ⟧ₘ ↑ • H ↑ • S^ -d/b ↑) ≈⟨ by-passoc (□ ^ 3 • □ ^ 2) (□ ^ 4 • □) auto ⟩
