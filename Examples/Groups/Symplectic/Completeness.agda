...
module Examples.Groups.Symplectic.Completeness (p-2 : ℕ) (p-prime : Prime (2+ p-2))  where
...
lemma-coset-update {n@2} lm@(inj₂ (d@(₀ , b) , lm↑)) g@S-gen = lm' , w , claim
  ...
  claim = begin
    ...
    (Ex • CZ^ (- b) • [ g ]ʷ) • [ lm↑ ]ˡᵐ ↑ ≈⟨ (cleft cright comm⇒pow-comm (toℕ (- b)) 1 (axiom comm-CZ-S↓)) ⟩
...
lemma-coset-update {n@2} lm@(inj₂ (d@(₀ , b@(₁₊ _)) , lm↑)) g@H-gen = lm' , w , claim
  ...
  claim = begin
    ...
    (Ex • ((ZM b* • CZ^ (- ₁)) • ZM b⁻¹ ) • H) • [ lm↑ ]ˡᵐ ↑ ≈⟨ by-passoc ((□ • (□ ^ 2 • □) • □) • □ ) (□ ^ 2 • □ • □ ^ 2 • □) auto ⟩
    (Ex • ZM b*) • CZ^ (- ₁) • (ZM b⁻¹ • H) • [ lm↑ ]ˡᵐ ↑ ≈⟨ (cleft {!lemma-Ex-M!}) ⟩
    (ZM b* ↑ • Ex) • CZ^ (- ₁) • (ZM b⁻¹ • H) • [ lm↑ ]ˡᵐ ↑ ≈⟨ by-passoc  (□ ^ 2 • □ • □ ^ 2 • □) (□ • (□ ^ 2 • □ ^ 2) • □) auto ⟩
