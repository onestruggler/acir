...
module Examples.Groups.Symplectic.Pushing.DH (p-2 : ℕ) (p-prime : Prime (2+ p-2))  where
...
aux-DH {n} d@(a@(₁₊ _) , b@(₁₊ _)) = trans claim claim'
  ...
  claim = begin
    ...
    (Ex • CZ^ (- ₁) • (ZM ((a , λ ()) ⁻¹) • H • S^ -b/a) • H) ≈⟨ ( cright cright by-passoc (□ ^ 3 • □) (□ ^ 2 • □ ^ 2) auto) ⟩
    ...
    (Ex • (CZ^ (- ₁) • S^ (-x⁻¹ * (y * y))) • ZM -y/x' • (H • S^ -x⁻¹)) ≈⟨ ( cright cleft comm⇒pow-comm (toℕ (- ₁)) (toℕ (-x⁻¹ * (y * y))) (axiom comm-CZ-S↓)) ⟩
    (Ex • (S^ (-x⁻¹ * (y * y)) • CZ^ (- ₁)) • ZM -y/x' • (H • S^ -x⁻¹)) ≈⟨ ( by-passoc (□ • □ ^ 2 • □) (□ ^ 2 • □ ^ 2) auto) ⟩
