...
module Examples.Groups.Symplectic.Normalization.Pushing.Push (p-2 : ℕ) (p-prime : Prime (2+ p-2)) where

------------------------------------------------------------------------
-- Pushing a dirty gate through the M column  [ m ]ᵐ • dirty ≈ dir • [ m' ]ᵐ

-- S through M (general n).
open import Examples.Groups.Symplectic.Normalization.Pushing.PushM p-2 p-prime public
  using (push-M-S)

-- S^j through M (general n), by iterating push-M-S.
open import Examples.Groups.Symplectic.Normalization.Pushing.PushMSn p-2 p-prime public
  using (push-M-Sⁿ)

-- S↑ through M (base M 2), CZ through M (M 3), and the E box absorbing an
-- S-power.
open import Examples.Groups.Symplectic.Normalization.Pushing.PushMScz p-2 p-prime public
  using (push-M-S↑ ; push-M-CZ ; push-M-CZ-M2-a0 ; push-E-S^)

-- H through M — the assembly, parametric in the (still-open) D-box H-push.
open import Examples.Groups.Symplectic.Normalization.Pushing.PushMH p-2 p-prime public
  using (push-M-H)

-- The a = 0 rows of that D-box H-push.
open import Examples.Groups.Symplectic.Normalization.Pushing.DHa0 p-2 p-prime public
  using (aux-DHa0)
...
open import Examples.Groups.Symplectic.Normalization.Pushing.PushLM2 p-2 p-prime public
