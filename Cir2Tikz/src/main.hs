...
-- Where the generated pictures go, relative to the directory the generator is
-- run from (the Cir2Tikz root).  \tikzfig falls back to ./figures/NAME.tikz, so
-- the \tikzfig{...} calls in the .tex files name pictures without this prefix.
figDir :: FilePath
figDir = "figures/"
...
writeGroup fname rels =
    writeFig fname (CT.tikz_of_rels $ map rel_trans rels)

-- Write a single picture to figures/NAME.tikz.
writeFig :: FilePath -> String -> IO ()
writeFig fname = writeFile (figDir ++ fname ++ ".tikz")
...
-- B↑←CZ : [b]ᵇ↑ • CZ ≈ dir • [b]ᵇ↑
-- Emitted as br3CZdBu: br3 = three-qupit box relation, CZd = CZ ↓ (the CZ on
-- the bottom wire pair), Bu = B box ↑ (shifted up one wire, so spanning 1-2).
rels_BCZ :: [Rel]
...
writeChain fname ch = writeFig ("chains/" ++ fname) (CT.tikz_of_pcir ch)
...
writeChainSplit n fname ch =
    mapM_ (\(i, seg) -> writeFig ("chains/" ++ fname ++ "_p" ++ show i)
                                 (CT.tikz_of_pcir seg))
...
main = do
    ...
    writeGroup "br3CZdBu" rels_BCZ
    writeGroup "ddcz" rels_DDCZ
    writeGroup "selinger"    rels_selinger
    writeGroup "hsh"         rels_hsh
    writeGroup "shczh"       rels_shczh
    writeFig   "shczh_chain" (CT.tikz_of_pcir proof_shczh)
    -- Layer-0 lemma library (top-level .tikz, referenced by test3.tex §Layer 0)
    writeFig   "lib_mmul"    (CT.tikz_of_pcir lib_mmul)
    writeFig   "lib_semims"  (CT.tikz_of_pcir lib_semims)
    writeFig   "lib_hhm"     (CT.tikz_of_pcir lib_hhm)
    -- E←S
