...
-- fixed point (respectively a spectator coordinate), and is injective.
--
-- Injectivity is not automatic, so it is not assumed: it is derived
-- from either of the two standard criteria, a trivial kernel or a
-- retraction, and then propagated to the k-fold embedding.
--
...
-- An embedding of each level into the next: a group homomorphism from
-- level n to level 1+n.  ("Embedding" names the intent; injectivity is
-- a separate result, established below from a kernel or a retraction.)
...
record Embedding (IG : IndexedGroup c ℓ) : Set (c ⊔ ℓ) where
  ...
  -- The homomorphism laws at level n: homo, ⁻¹-homo, ε-homo, ⟦⟧-cong.
  module Homo (n : ℕ) =
    GroupMorphisms.IsGroupHomomorphism (isGroupHomomorphism n)
...
module _ {IG : IndexedGroup c ℓ} (E : Embedding IG) where
  open IndexedGroup IG
  open Embedding E

  -- Injectivity of one step propagates to the k-fold embedding: peel
  -- off one embedding at a time.
  emb-injective⇒emb^-injective :
    (∀ {n} → Injective (_≈_ {n}) (_≈_ {suc n}) emb) →
    ∀ {n} k → Injective (_≈_ {n}) (_≈_ {k + n}) (emb^ k)
  emb-injective⇒emb^-injective inj zero    eq = eq
  emb-injective⇒emb^-injective inj (suc k) eq =
    emb-injective⇒emb^-injective inj k (inj eq)

  -- Levelwise injectivity upgrades each level's homomorphism to a
  -- monomorphism in the sense of Algebra.Morphism.Structures.
  isGroupMonomorphism :
    (∀ {n} → Injective (_≈_ {n}) (_≈_ {suc n}) emb) →
    ∀ n → GroupMorphisms.IsGroupMonomorphism
            (rawGroup n) (rawGroup (suc n)) (emb {n})
  isGroupMonomorphism inj n = record
    { isGroupHomomorphism = isGroupHomomorphism n
    ; injective           = inj
    }
  ...
  -- x ∙ y ⁻¹ ≈ ε, so x ≈ y.
  module TrivialKernel
    (kernel-trivial : ∀ {n} {x : Carrier n} →
                      emb x ≈ Group.ε (group (suc n)) → x ≈ Group.ε (group n))
    where

    emb-injective : ∀ {n} → Injective (_≈_ {n}) (_≈_ {suc n}) emb
    emb-injective {n} {x} {y} eq =
      A.trans (inverseˡ-unique x (y A.⁻¹) kernel-eq) (⁻¹-involutive y)
      where
      ...
      kernel-eq : (x A.∙ (y A.⁻¹)) A.≈ A.ε
      kernel-eq = kernel-trivial emb-x∙y⁻¹

    emb^-injective : ∀ {n} k → Injective (_≈_ {n}) (_≈_ {k + n}) (emb^ k)
    emb^-injective = emb-injective⇒emb^-injective emb-injective
  ...
  module Retraction
    ...
    (retract-cong : ∀ {n} {x y : Carrier (suc n)} → x ≈ y → retract x ≈ retract y)
    (retract-emb  : ∀ {n} (x : Carrier n) → retract (emb x) ≈ x)
    where

    emb-injective : ∀ {n} → Injective (_≈_ {n}) (_≈_ {suc n}) emb
    emb-injective {n} {x} {y} eq = begin
