...
\item \emph{Certificates are first-order.}  A verified coset table is finite
  ...
  (\S\ref{sec:engine}); obligations are therefore finite case splits, and in
  the case studies they close by evaluation.
...
\AgdaOperator{\AgdaFunction{\AgdaUnderscore{}↥ᵏ\AgdaUnderscore{}}}\<%
\\
%
\\[\AgdaEmptyExtraSkip]%
...
wire-indexed generators \aid{Gen} and the circuits $\aid{Circuit}\;n =
\aid{Word}\;(\aid{Gen}\;n)$ exactly as shown in \S\ref{sec:permutations}:
a generator is a 1- or 2-ary gate at the bottom of the wires, shifted up
by some number of \aid{{\_}↥}s, and $c{\uparrow} = \aid{wmap}\;\aid{{\_}↥}$
...
$\aid{Gen}\,(2{+}n)$, not in $\aid{Gen}\,k$ with a side condition $k \ge
2$ --- and the $k$-fold shift puts $k$ on the \emph{left} of $+$, mirroring
the left-biased inductive definition of addition.  The module's own
comment states the reason:

\begin{code}%
\>[0]\AgdaComment{--\ Indices\ begin\ with\ suc\ (no\ top-level\ addition)\ so\ Agda's\ coverage}\<%
\\
\>[0]\AgdaComment{--\ checker\ can\ solve\ unification\ goals\ by\ injectivity\ of\ ₁₊\ alone,}\<%
\\
\>[0]\AgdaComment{--\ avoiding\ stuck\ Diophantine\ equations\ of\ the\ form\ arity\ +\ k\ ≟\ target.}\<%
\\
\>[0]\AgdaKeyword{data}\AgdaSpace{}%
...
\\
\>[0][@{}l@{\AgdaIndent{0}}]%
\>[2]\AgdaComment{--\ gate₁,\ gate₂\ and\ \AgdaUnderscore{}↥,\ as\ in\ Section\ 2.1}\<%
\\
%
\\[\AgdaEmptyExtraSkip]%
\>[0]\AgdaOperator{\AgdaFunction{\AgdaUnderscore{}↥ᵏ\AgdaUnderscore{}}}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaDatatype{Gen}\AgdaSpace{}%
...
\>[41]\AgdaComment{--\ k\ on\ the\ left\ of\ +}\<%
\end{code}
\begin{code}[hide]%
\>[0]\AgdaBound{g}\AgdaSpace{}%
\AgdaOperator{\AgdaFunction{↥ᵏ}}\AgdaSpace{}%
...
defined by direct pattern matching and verified by \aid{refl}:
unification alone discharges the well-formedness constraints that arise,
and no arithmetic side condition ever contaminates a table.  Second, the
structural rules common to \emph{all} circuit theories are factored out
once, by \aid{Lift-Relation}, whose permutation instance
\aid{{\_}VRel,{\_}==={\_}} appeared in \S\ref{sec:permutations}: given any
gate-specific family $\aid{{\_}SRel,{\_}==={\_}} : (n : \mathbb{N}) \to
\aid{CRel}\;n$, it adds shift-congruence and far-commutation (a bottom
gate commutes with any generator shifted past its arity).  The
accompanying lemma \aid{lemma-cong↑} lifts the \emph{entire congruence}
one wire up --- proved once, by induction over the eight constructors of
$\approx$ --- so equational developments can move freely along the wire
chain $\mathsf{Cir}\,k \le \mathsf{Cir}\,(k{+}1)$ of
...
injectivity theorems for extensions of generator maps, in two strengths.
The \emph{simplified} form needs no cosets: if $f : X \to \aid{Word}\;Y$
has a generator-level retraction $g : Y \to \aid{Word}\;X$ ---
...
\begin{code}%
\>[0]\AgdaKeyword{module}\AgdaSpace{}%
\AgdaModule{SingleLevel}\<%
\\
\>[0][@{}l@{\AgdaIndent{0}}]%
\>[2]\AgdaSymbol{(}\AgdaBound{Γ}%
\>[7]\AgdaSymbol{:}\AgdaSpace{}%
\AgdaFunction{WRel}\AgdaSpace{}%
\AgdaPostulate{X}\AgdaSymbol{)}%
\>[31]\AgdaComment{--\ subgroup\ presentation\ \ (letters\ X)}\<%
\\
%
\>[2]\AgdaSymbol{(}\AgdaBound{Δ}%
\>[7]\AgdaSymbol{:}\AgdaSpace{}%
\AgdaFunction{WRel}\AgdaSpace{}%
\AgdaPostulate{Y}\AgdaSymbol{)}%
\>[31]\AgdaComment{--\ group\ presentation\ \ \ \ \ (letters\ Y)}\<%
\\
%
\>[2]\AgdaSymbol{(}\AgdaBound{C}%
\>[7]\AgdaSymbol{:}\AgdaSpace{}%
\AgdaPrimitive{Set}\AgdaSymbol{)}%
\>[31]\AgdaComment{--\ set\ of\ right\ cosets}\<%
\\
%
\>[2]\AgdaSymbol{(}\AgdaBound{I}%
\>[7]\AgdaSymbol{:}\AgdaSpace{}%
\AgdaBound{C}\AgdaSymbol{)}%
\>[31]\AgdaComment{--\ identity\ coset}\<%
\\
%
...
\AgdaDatatype{Word}\AgdaSpace{}%
\AgdaPostulate{Y}\AgdaSymbol{)}%
\>[31]\AgdaComment{--\ generator\ embedding\ \ H\ ↪\ G}\<%
\\
%
\>[2]\AgdaSymbol{(}\AgdaBound{h}%
\>[7]\AgdaSymbol{:}\AgdaSpace{}%
...
\AgdaOperator{\AgdaFunction{×}}\AgdaSpace{}%
\AgdaBound{C}\AgdaSymbol{)}%
\>[31]\AgdaComment{--\ coset\ action\ (Schreier\ table)}\<%
\\
%
\>[2]\AgdaSymbol{(}\AgdaOperator{\AgdaBound{[\AgdaUnderscore{}]}}\AgdaSpace{}%
\AgdaSymbol{:}\AgdaSpace{}%
\AgdaBound{C}\AgdaSpace{}%
\AgdaSymbol{→}\AgdaSpace{}%
\AgdaDatatype{Word}\AgdaSpace{}%
\AgdaPostulate{Y}\AgdaSymbol{)}%
\>[31]\AgdaComment{--\ Schreier\ section}\<%
\end{code}
\begin{code}[hide]%
%
\>[2]\AgdaKeyword{where}\<%
...
coset presentation --- quoted from the source, numbered as in the
...
\[
\begin{array}{l}
\aid{dpres} : (\Gamma \mathrel{\aid{⋄}} \Delta \mathrel{\aid{⋄}} \aid{CommRel})
   \ \aid{IsPresentationOf}\ (G_1 \times G_2) \\[3pt]
\aid{dpres} : (\Gamma \mathrel{\aid{⋄}} \Delta \mathrel{\aid{⋄}} \aid{ConjRelʷ}\;\mathit{conj})
...
(cf.\ Proposition~2.55 of~\cite{holt2005handbook}); semidirect products are
...
necessary for some semantic side of a presentation theorem;
\S\ref{sec:stdlib-gap} draws the moral.
...
this turns table verification into evaluation.  Neither tool is a tactic
in the metaprogramming sense: each is a verified function into a
canonical data type followed by \aid{refl}, the small-scale-reflection
recipe with the reflection left to the evaluator (\S\ref{sec:whyagda}).
