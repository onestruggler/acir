...
every function out of words carries a congruence proof, and the standard
...
tails --- are natural for setoids and not in general definable out of a
quotient; (iii) the library stays inside plain Agda and the standard
library's algebra hierarchy, a requirement for upstreaming; (iv) the
congruence $\approx$ is itself an inductive family, so the
congruence-lifting and transport lemmas of \S\ref{sec:constructions} and
the wire-shift lemma of \S\ref{sec:circuits} are inductions on its
derivations --- relation sets, the objects this library exists to
manipulate, are first-class, whereas a quotient absorbs the relation into
the identity type and leaves nothing to induct on.  The univalent
...
but the quotient's eliminator still demands a respect proof for every
function out of it, and (ii), (iii), and (iv) would be forfeited.  For a
...
from the literature, not formalized.
