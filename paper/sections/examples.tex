...
\subsection{Pauli groups in all odd prime dimensions}
...
The third case study formalizes the $n=3$ instance of Bian and Selinger's
presentation of $U_n(\mathbb{Z}[\tfrac12,i])$~\cite{bian2021un} --- the
unitary matrices over the ring $\mathbb{Z}[\tfrac12,i]$, the natural home
of the Clifford+$T$-with-ancilla gate set
...
$\sim$68\,000 lines formalizing the road to multi-qupit Clifford
...
companion web normalizer of \S\ref{sec:tools}~\cite{qupit-tool}.  This
layer is not yet complete (its remaining obligations concern the
equivalence of two Pauli-group carriers), and in keeping with our
ground rule --- nothing is claimed that does not typecheck under
\texttt{--safe} today --- we report it as the library's driving next
target rather than as a result.  What \emph{is} already visible from the
verified layers is the intended endgame: the Pauli presentations of this
section as the normal subgroup, the symplectic staircase as the coset
tower, and the extension-presentation theorem of
\S\ref{sec:constructions} as the glue.
