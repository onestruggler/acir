...
which is exactly the carrier $\aid{NF}\;n = \top \times C\,2 \times \dots
\times C\,n$ built by the coset tower in \S\ref{sec:permutations}.  For a
...
generators and a set $R$ of pairs of words over $X$ (we work with monoid
presentations, so words are finite sequences with concatenation, and there
are no formal inverses).  The presented monoid is the quotient of the free
monoid $X^{*}$ by the least congruence $\approx$ containing $R$; the
\emph{word problem} asks to decide $w \approx v$.  A presentation
\emph{presents} a monoid or group $G$ (relative to an interpretation of
generators) when the induced homomorphism $X^{*}/{\approx} \to G$ is an
isomorphism; its injectivity is \emph{completeness} of $R$ for $G$, its
surjectivity says the generators generate.  A group presentation in the
...
holt2005handbook}); the library instead takes a \emph{claimed} coset table
as input and \emph{verifies} it, by exhibiting five equational
...
the library turns it into a reusable module with the hypotheses packaged as
a record.
...
Figure~\ref{fig:staircase}; for Clifford-style groups, Selinger's
...
classical alternating normal form~\cite{magnus2004combinatorial} the
library also verifies (\S\ref{sec:constructions}).
