...
\item \emph{Specifications are standard-library bundles.}  ``Normal form''
  ...
  no new lemmas to maintain.
\item \emph{Certificates are finite.}  A verified coset table is finite
  data plus five equations, three quantified over finite types and two
  over axiom instances (\S\ref{sec:engine}); obligations are therefore
  case splits --- which must still be made --- whose cases are closed
  goals, discharged by evaluation where they reduce to a computation and
  by short equational derivations otherwise.
...
\item \emph{Constructions come in pairs.}  Every product construction has a
  syntactic theorem (normal-form witnesses for the factors combine into
  one for the product) and a semantic one (the constructed relation set
  presents the constructed group), and the semantic groups are
...
\noindent so \aid{(f ʷ)} is ``the unique monoid homomorphism extending $f$''.  Two
...
\noindent together with its right-to-left mirror; and a small family of
...
\S\ref{sec:setoids} weighs that choice.
...
\noindent i.e.\ a grouplike witness plus a group isomorphism from the presented
...
\emph{exactness}, $\aid{nf} \circ \aid{inv-nf} \approx \mathrm{id}$ ---
...
careful about.  Classically, an injective $\aid{nf}$ has a section on
its image for free, by choosing a preimage for each normal form;
constructively, choosing is not free, because \emph{the section is
data}.  An \aid{Injection} gives no algorithm
...
statement about \emph{normal forms}, i.e.\ about concrete finite data
--- never about arbitrary words.  Two companion lemmas follow the same
pattern.  \aid{by-normalization-wsurj} reduces surjectivity of
$\aid{⟦\_⟧}$ (every group element is denoted by some word) to
surjectivity on normal forms (every group element is denoted by some
canonical representative).  \aid{by-completeness} runs the main lemma
backwards: from completeness plus an exact section ($\aid{nf} \circ
\aid{inv\text{-}nf} \approx \mathrm{id}$) it recovers
\aid{UniqueNormalForm}, which is how the construction theorems of
\S\ref{sec:constructions} lift uniqueness witnesses through products,
semidirect products, and amalgamations (Table~\ref{tab:nfs}).
Sub-presentations then come
...
has type $\aid{Gen}\;n \to \aid{Gen}\;(k + n)$.  With this choice,
every index equation the coverage checker meets is solved by
unification alone, through injectivity of \aid{₁₊}, never by
arithmetic; the stuck Diophantine equations of the form $\mathit{arity}
+ k \doteq \mathit{target}$ that the other choice produces never arise.
Downstream, this is why the coset tables of the case studies can be
defined by direct pattern matching.  Second, the structural rules common to all circuit theories
...
\aid{Transfer} takes the five hypotheses that certify the data as a
coset table for $\Gamma$ inside $\Delta$:
...
hypotheses into a single value, so that a verified level can be passed
around like any other value; the amalgamation construction of
\S\ref{sec:constructions} takes two such records as its input, and
\aid{CosetTower} iterates levels up an
$\mathbb{N}$-indexed family of presentations, one such record per
level, with carrier
...
\S\ref{sec:permutations}.  This is how the framework takes over part of
the ``gigantic computations'' of \S\ref{sec:intro}.  The case splits
remain and must still be written out, but the machine now carries two
of their burdens: coverage checking guarantees that no case is missed,
and the cases that are mere computation are closed by evaluation at no
cost.
...
normal-form transports carry witnesses across these maps: a normal form
for one factor becomes a weak normal form for the union, and a normal
form for a presented monoid pulls back along any monoid monomorphism
into it.
...
direct and semidirect products.  The amalgamated product starts not
from normal forms for the factors but from a normal form for the common
subgroup, with carrier $\mathit{NF}_0$, together with a verified coset
table for each factor over that subgroup, with coset types $C$ and $D$;
its carrier is $\mathit{NF}_0 \times \aid{amalNFC}\;C\;D$, where the
coset part
\begin{code}[hide]
amalNFC : Set → Set → Set
\end{code}
...
\end{code}
is the classical alternating carrier, a word of alternating nontrivial
coset representatives between two
...
$M \ast_C N$ of \emph{monoids} as a setoid rather than a set: elements are
...
needed for the semantic side of at least one presentation theorem.  The
pattern is telling: the standard library's algebra hierarchy is rich in
\emph{structures} --- monoids, groups, their morphisms --- but thin in
\emph{constructions} that build new ones from old, and a presentation
theorem cannot be stated without the group it presents, so each
construction had to be built before its theorem could be.

\subsection{Proof engineering: solvers, evaluation, and notation}\label{sec:solvers}
...
evaluation.  All three are used like tactics, but none uses
metaprogramming: each maps both sides of a goal into a canonical data
type by a function proved correct once, and then compares the results
with \aid{refl}, so the evaluator does the work that a reflective tactic
would do by proof search.
