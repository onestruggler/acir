...
- **`Trivial/`**: the trivial group, two presentations proved isomorphic. Everything is derived once from the single hypothesis that every generator is `≈ ε` (`gen≈ε`), then instantiated twice. `Syntactics` is `⟨ ⊥ ∣ ⟩` (`EmptyRel`) and `Syntactics-Alt A` is `⟨ A ∣ w = ε ⟩` (`TrivialRel`) — each supplies only `pres` and `gen≈ε`. Parameterised by those two: `Normalization` (`w≈ε`, `nf`/`nfp`/`nfp'`), `Interpretation` (`⟦_⟧₀`, `GS`, `fʷ-cong-ax`, `grouplike`), `UniqueNormalForm` (`unfp`). `Semantics` is the shared target (`Terminal.group`, as `gp`). `Presentation` and `Presentation-Alt A` instantiate the lot and state `IsPresentationOf`; `Presentation-Equivalence B` gives the monoid isomorphism between the two. Note `gp` is re-exported, not abstracted over the module parameter, so it is `Presentation-Alt.gp`, never `.gp A`.
