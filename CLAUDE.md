...
Amy's path-sum calculus (QPL 2018): the parts of §§2–4 that corollary 4.4 needs — lemmas 4.1–4.3, and corollary 4.4 for circuits over {H, S, CZ} as the paper states it, about the circuit itself and syntactically (`corollary-4-4-syntactic`). **Not reached by `MainTheorems.agda`**; its own root is `PathSum/Theorems.agda`, which covers the whole directory. Typecheck it with `wsl --exec /usr/bin/agda +RTS -M10G -RTS PathSum/Theorems.agda` — **always under a heap cap**: `Denotation` alone peaks near 10 GB and takes ~250–270 s CPU, and an uncapped run once exhausted the WSL VM. `Cyclotomic` takes ~2 min; everything else is cheap (well under a minute together).
...
- **`Assign.agda`**: shared by `AssignSum`, `CircuitAmp`, `CircuitSemantics`, `Isometry` and `Decide` — `_[_≔_]` (overwrite one bit) and its pointwise laws, `_=ᵇ_`, `same` (two assignments agree everywhere), `[_]ᶻ`.
...
- **`Syntactic.agda`**: the converse of `Identity.id-if` — a path-sum without path variables that is the identity has no normalisation, outputs the inputs mod 2 and phase 0 mod 2^M, *coefficient by coefficient* (`id-only-if`, via `Decide.id⇔′`, the mod-2 reading `outBit-∣`, and `Mobius`); together `id⇔syntactic`.
