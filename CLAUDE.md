...
Amy's path-sum calculus (QPL 2018), through §4: lemmas 4.1–4.3 and corollary 4.4, the last stated about the circuit itself. **Not reached by `MainTheorems.agda`**; its own root is `PathSum/Theorems.agda`, which covers the whole directory. Typecheck it the same way (`wsl --exec /usr/bin/agda PathSum/Theorems.agda`), **always under a heap cap** (`agda +RTS -M10G -RTS …`): `Denotation` alone peaks near 10 GB and takes ~270 s CPU, and an uncapped run once exhausted the WSL VM. Everything else in the directory is cheap (the six modules behind lemma 4.1 check in well under a minute together).
...
- **`CircuitSemantics.agda`**: `Column`, `δ`, `gateᴬ`, `applyᴬ` (the circuit's matrix, gate by gate); `prop-2-10` (`amp ⟦ C ⟧ x z ≐ applyᴬ C (δ x) z`), `⟦⟧-unit-columns` (every column of `⟦ C ⟧` has trace-form norm 2^(norm C); orthogonality, i.e. unitarity, is not stated), `⟦⟧ᴿ-restricts` (same diagonal as `⟦ C ⟧`) and `⟦⟧ᴿ-diagonal`.
- **`Decide.agda`**: the test on a fully reduced path-sum. `id-if′`, the identity criterion *input by input* (no normalisation, the single path returns every x, phase ≡ 0 mod 2^M at every x) — unlike `Identity.id-if`, which is coefficient-wise and so does not visibly meet the pointwise refutations; `search`, a finite search over assignments for any decidable property that respects pointwise equality; and `decide-≋-id : Dec (ξ ≋ idPS)` for `ξ` without path variables.
