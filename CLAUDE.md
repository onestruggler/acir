...
- **`Assign.agda`**: shared by everything below — `_[_≔_]` (overwrite one bit) and its pointwise laws, `_=ᵇ_`, `same` (two assignments agree everywhere), `[_]ᶻ`.
...
- **`CircuitSemantics.agda`**: `Column`, `δ`, `gateᴬ`, `applyᴬ` (the circuit's matrix, gate by gate); `prop-2-10` (`amp ⟦ C ⟧ x z ≐ applyᴬ C (δ x) z`), `⟦⟧-isometry` (every column of `⟦ C ⟧` has norm 2^(norm C)), `⟦⟧ᴿ-restricts` (same diagonal as `⟦ C ⟧`) and `⟦⟧ᴿ-diagonal`.
...
- **`Theorems.agda`**: instantiates `Clifford` at `Denotation.semantics` and states everything unconditionally — `prop-2-10`, `circuit-isometry`, `circuit-≋-id` (`⟦ C ⟧ ≋ idPS` iff the circuit's matrix is the identity), `lemma-4-1`, `lemma-4-1-circuit` (`⟦ C ⟧ ≋ idPS ⇔ ⟦ C ⟧ᴿ ≋ idPS`), `corollary-4-4-circuit` (about `⟦ C ⟧ᴿ`) and `corollary-4-4-⟦⟧` (about `⟦ C ⟧`: either a refutation, or a reduct with no path variables whose being the identity is exactly the circuit's), `reduct≋`, and `circuit-id`/`circuit-not-id`, which carry the verdict on a reduct back to `⟦ C ⟧`.

Nothing is assumed: the hypotheses of the corollary are proved by `Circuit`, the step back from the restriction by lemma 4.1 (`Isometry` + `CircuitSemantics`), and the verdict at the end of a reduction by `Identity`. The one link that is argued in prose rather than formalised is that definition 2.4 (a partial isometry) implies `WellFormed`; the lemma only ever uses `WellFormed`, and circuits satisfy it by `circuit-isometry`, so no theorem depends on the prose.

**Denotation's public surface.** Its helpers are private; what other modules may use is `amp-idPS`, `hits-cong`, and the section *Reading the outputs at a path* (`outBit`, `hits-intro`/`hits-elim`, `hits-≗`, `amp-≗`, `outBit-μ`, `eval-μ-val`, `eval-0ᴾ-val`). Adding to that file costs a ~5 min, ~10 GB recheck, so prototype against it, not in it.
