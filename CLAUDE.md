...
Amy's path-sum calculus (QPL 2018): the parts of §§2–4 that corollary 4.4 needs — lemmas 4.1–4.3, and corollary 4.4 for circuits over {H, S, CZ} as the paper *proves* it, about the circuit itself and syntactically (`corollary-4-4-any`, `corollary-4-4-syntactic`); its polynomial-time statement is not formalised. **Not reached by `MainTheorems.agda`**; its own root is `PathSum/Theorems.agda`, which covers the whole directory. Typecheck it with `wsl --exec /usr/bin/agda +RTS -M10G -RTS PathSum/Theorems.agda` — **always under a heap cap**: `Denotation` alone peaks near 10 GB and takes ~250–270 s CPU, and an uncapped run once exhausted the WSL VM. `Cyclotomic` takes ~2 min; everything else is cheap (well under a minute together).
...
- **`Reduction.agda`**: `⅛`/`¼`/`½`, the reducts, and `_⟶_` with constructors `elim`, `ω`, `hh` — each on the first path variable, with Z₂-linear quotients (`liftXor c S`), which is all lemma 4.3 needs. `[Case]` is deliberately absent.
...
- **`CircuitSemantics.agda`**: `Column`, `δ`, `gateᴬ`, `applyᴬ` (the circuit's matrix, gate by gate); `prop-2-10` (`amp ⟦ C ⟧ x z ≐ applyᴬ C (δ x) z`), `⟦⟧-unit-columns` (every column of `⟦ C ⟧` has trace-form norm 2^(norm C); unitarity is not stated), `⟦⟧ᴿ-restricts` (same diagonal as `⟦ C ⟧`) and `⟦⟧ᴿ-diagonal`.
...
- **`Isometry.agda`**: lemma 4.1 for any path-sum. `WellFormed ξ` = every column has trace-form norm ≤ 2^k (implied by definition 2.4 — the Galois argument is in the header, as prose); `Restriction-id ξ` = `amp ξ x x ≐ scale k (zpow 0ℤ)` for all x (the restricted sum is the normalised 1); `lemma-4-1`, and `diagonal-≋` for path-sums none of whose paths leaves its input.
...
Departures from the paper (all in the module headers; the not-formalised list is in the paragraph above): a path-sum carries its normalisation `k` **and** its path-variable count `m` separately, because def. 2.1 ties them but fig. 2's rules do not; lemma 4.3's implicit side condition (the rule may cost more normalisation than the path-sum has) is *proved* here rather than assumed, via the coordinate facts above; circuits are over {H, S, CZ} rather than {H, CNOT, R_k}; and lemma 4.1 is proved under `WellFormed` (trace-form column norms) rather than definition 2.4.
