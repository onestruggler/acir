...
anfpₗ : ∀ {A} {Γ Δ : WRel A} → PP.NormalFormWithoutInverse Γ → PP.WeakNormalForm (Γ ∪ Δ)
...
anfpₗ' : ∀ {A} {Γ Δ : WRel A} → PP.WeakNormalForm Γ → PP.WeakNormalForm (Γ ∪ Δ)
anfpₗ' {A} {Γ} {Δ} anfp = record { ANF = ANF ; anf = anf ; anf-injective = λ x → lefts (anf-injective x) }
  where
  open PP.WeakNormalForm anfp
  open LeftRightCongruence-∪ Γ Δ

anfpᵣ : ∀ {A} {Γ Δ : WRel A} → PP.NormalFormWithoutInverse Δ → PP.WeakNormalForm (Γ ∪ Δ)
anfpᵣ {A} {Γ} {Δ} nfp = record { ANF = NF ; anf = nf ; anf-injective = λ x → rights (nf-injective x) }
  where
  open PP.NormalFormWithoutInverse nfp
  open LeftRightCongruence-∪ Γ Δ

anfpᵣ' : ∀ {A} {Γ Δ : WRel A} → PP.WeakNormalForm Δ → PP.WeakNormalForm (Γ ∪ Δ)
...
mono-anfp : ∀ {A B} {Γ : WRel A} {Δ : WRel B} → PP.WeakNormalForm Δ → (f : Word A → Word B) →
  let open PP Γ renaming (•-ε-monoid to m₁) in
  let open PP Δ renaming (•-ε-monoid to m₂) in
  MonoidMorphisms.IsMonoidMonomorphism (Monoid.rawMonoid m₁) ((Monoid.rawMonoid m₂)) f → PP.WeakNormalForm (Γ)
...
mono-nfp : ∀ {A B} {Γ : WRel A} {Δ : WRel B} → PP.NormalFormWithoutInverse Δ → (f : Word A → Word B) →
  let open PP Γ renaming (•-ε-monoid to m₁) in
  let open PP Δ renaming (•-ε-monoid to m₂) in
  MonoidMorphisms.IsMonoidMonomorphism (Monoid.rawMonoid m₁) ((Monoid.rawMonoid m₂)) f → PP.NormalFormWithoutInverse (Γ)
...
iso-nfp' : ∀ {A B} {Γ : WRel A} {Δ : WRel B} → PP.NormalForm Δ → (f : Word A → Word B) →
  let open PP Γ renaming (•-ε-monoid to m₁) in
  let open PP Δ renaming (•-ε-monoid to m₂) in
  MonoidMorphisms.IsMonoidIsomorphism (Monoid.rawMonoid m₁) ((Monoid.rawMonoid m₂)) f → PP.NormalForm (Γ)
