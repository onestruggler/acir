...
module SNFP
  ...
  open BijectiveNormalForm nfp-Γ renaming
    (hasNormalForm to nfp-Γ' ; NF to NF₁ ; nf to nf₁ ; nf-injective to nf₁-inj ; nf-cong to nf₁-cong ; inv-nf to inv-nf₁ ; inv-nf∘nf=id to inv-nf₁∘nf₁=id) using ()
  open BijectiveNormalForm nfp-Δ renaming
    (hasNormalForm to nfp-Δ' ; NF to NF₂ ; nf to nf₂ ; nf-injective to nf₂-inj ; nf-cong to nf₂-cong ; inv-nf to inv-nf₂ ; inv-nf∘nf=id to inv-nf₂∘nf₂=id) using ()
...
{-
  ...
  snfp : BijectiveNormalForm (Γ ⸲ Δ ⸲ Γₓ)
  snfp = record
           { NF = NF
           ; nf = nf
           ; nf-cong = nf-cong
