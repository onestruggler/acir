...
module Presentation.Groups.S16b where
  ...
  lemma-fm³ {n'} [ inj₂ swap ]ʷ = bef Eq.refl
    where
    n = 2+ (₁₊ n')
    open NormalFormWithoutInverse (DP.NFP.nfp (pres 1) (pres n) (nfp 1) (nfp n)) renaming (by-equal-nf to bef)
  ...
  h-wd-ax {n@(₁₊ nn)} cd@(swap• c , swap• d) {u} {t} ( (congₛ {w = w} {v} eq)) with h-wd-ax (c , d) (eq)
  ... | (ihl , ihr) rewrite  ihr = 
  ...
  h-wd-ax {n@zero} (swap• ε , swap• ε) yang-baxter = (bef Eq.refl) , Eq.refl
    where open NormalFormWithoutInverse (DP.NFP.nfp (pres 1) (pres n) (nfp 1) (nfp n)) renaming (by-equal-nf to bef)

  h-wd-ax {zero} (swap• ε , ε) (congₛ order) = left-unit , Eq.refl
  h-wd-ax {₁₊ n} (swap• ε , ε) order = left-unit , Eq.refl
  h-wd-ax {₁₊ n} (swap• ε , swap• ε) order = left-unit , Eq.refl
  h-wd-ax {₁₊ n} (swap• swap• c , ε) order = left-unit , Eq.refl
  h-wd-ax {₁₊ n} (swap• swap• c , swap• ε) order = left-unit , Eq.refl
  h-wd-ax {₁₊ n} (swap• swap• c , swap• swap• d) order = axiom (right order) , Eq.refl
  h-wd-ax {₁₊ n} (swap• ε , ε) yang-baxter = refl , Eq.refl
  h-wd-ax {₁₊ n} (swap• ε , swap• ε) yang-baxter = by-assoc Eq.refl , Eq.refl
    where open PP (pres 1 ⊕ pres (₁₊ n))
  h-wd-ax {₁₊ n} (swap• swap• c , ε) yang-baxter = by-assoc Eq.refl , Eq.refl
    where open PP (pres 1 ⊕ pres (₁₊ n))
  h-wd-ax {₁₊ n} (swap• swap• ε , swap• ε) yang-baxter = by-assoc Eq.refl , Eq.refl
    where open PP (pres 1 ⊕ pres (₁₊ n))
  h-wd-ax {₁₊ n} (swap• swap• ε , swap• swap• ε) yang-baxter = by-assoc Eq.refl , Eq.refl
    where open PP (pres 1 ⊕ pres (₁₊ n))
  h-wd-ax {₁₊ n} (swap• swap• swap• c , swap• ε) yang-baxter = by-assoc Eq.refl , Eq.refl
    where open PP (pres 1 ⊕ pres (₁₊ n))
  h-wd-ax {₁₊ n} (swap• swap• swap• c , swap• swap• ε) yang-baxter = by-assoc Eq.refl , Eq.refl
    where open PP (pres 1 ⊕ pres (₁₊ n))
  h-wd-ax {₁₊ n} (swap• swap• swap• ε , swap• swap• swap• ε) yang-baxter = by-assoc-and (axiom (right yang-baxter)) Eq.refl  Eq.refl , Eq.refl
    where
    open NormalFormWithoutInverse (DP.NFP.nfp (pres 1) (pres (₁₊ n)) (nfp 1) (nfp (₁₊ n))) renaming (by-equal-nf to bef)
    open PP (pres 1 ⊕ pres (₁₊ n)) using (by-assoc ; by-assoc-and)

  h-wd-ax {₁₊ n} (swap• swap• swap• swap• c , swap• swap• swap• ε) yang-baxter = by-assoc-and (axiom (right yang-baxter)) Eq.refl Eq.refl , Eq.refl
    where
    open NormalFormWithoutInverse (DP.NFP.nfp (pres 1) (pres (₁₊ n)) (nfp 1) (nfp (₁₊ n))) renaming (by-equal-nf to bef)
    open PP (pres 1 ⊕ pres (₁₊ n)) using (by-assoc ; by-assoc-and)
    
  h-wd-ax {₁₊ n} (swap• swap• swap• swap• c , swap• swap• swap• swap• d) yang-baxter = by-assoc-and (axiom (right yang-baxter)) Eq.refl Eq.refl , Eq.refl
    where open PP (pres 1 ⊕ pres (₁₊ n))


  h-wd-ax {₁₊ n} (swap• ε , ε) (congₛ order) = left-unit , Eq.refl
  h-wd-ax {₁₊ n} (swap• ε , ε) (congₛ (comm {a = swap})) = trans left-unit (sym right-unit) , Eq.refl
  h-wd-ax {₁₊ n} (swap• ε , ε) (congₛ (comm {a = swap ₛ})) = trans left-unit (sym right-unit) , Eq.refl
  h-wd-ax {₁₊ n} (swap• ε , ε) (congₛ (comm {a = (a ₛ) ₛ})) = trans left-unit (sym right-unit) , Eq.refl
  h-wd-ax {₁₊ n} (swap• ε , ε) (congₛ yang-baxter) = refl , Eq.refl
  h-wd-ax {₁₊ n} (swap• ε , ε) (congₛ (congₛ order)) = left-unit , Eq.refl
  h-wd-ax {₁₊ n} (swap• ε , ε) (congₛ (congₛ comm)) = trans left-unit (sym right-unit) , Eq.refl
  h-wd-ax {₁₊ n} (swap• ε , ε) (congₛ (congₛ yang-baxter)) = (by-assoc Eq.refl) , Eq.refl
    where
    open NormalFormWithoutInverse (DP.NFP.nfp (pres 1) (pres (₁₊ n)) (nfp 1) (nfp (₁₊ n))) renaming (by-equal-nf to bef)
    open PP (pres 1 ⊕ pres (₁₊ n)) using (by-assoc ; by-assoc-and)
  ...
  h-wd-ax {₁₊ n} (swap• swap• c , ε) (congₛ order) = axiom (right order) , Eq.refl
  h-wd-ax {₁₊ n} (swap• swap• ε , ε) (congₛ yang-baxter) = (by-assoc Eq.refl) , Eq.refl
    where
    open PP (pres 1 ⊕ pres (₁₊ n)) using (by-assoc ; by-assoc-and)
  h-wd-ax {₁₊ n} (swap• swap• swap• c , ε) (congₛ yang-baxter) = by-assoc-and (axiom (right yang-baxter)) Eq.refl Eq.refl , Eq.refl
    where
    open PP (pres 1 ⊕ pres (₁₊ n)) using (by-assoc ; by-assoc-and)
  ...
  h-wd-ax {n@(₂₊ nn)} cd@(swap• swap• c , d@ε) {u} {t} (congₛ (congₛ (congₛ {w = w} {v} eq))) with h-wd-ax (c , ε) (congₛ eq)
  ... | (ihl , ihr) rewrite  ihr = 
  ...
  h-wd-ax {2+ n} (swap• swap• swap• ε , ε) (congₛ (congₛ (comm {a = (a ₛ) ₛ}))) = by-assoc-and (axiom (right (congₛ comm)))  Eq.refl Eq.refl , Eq.refl
    where
     open PP (pres 1 ⊕ pres (2+ n)) renaming (word-setoid to ws)
  h-wd-ax {n@(2+ (₁₊ nn))} (swap• swap• swap• swap• c , ε) (congₛ (congₛ (comm {a = a}))) =
  ...
  h-wd-ax {n@(₂₊ nn)} (swap• c@(swap• c0) , swap• d@(swap• d0)) (comm {a = (a)}) = 
  ...
  h-wd-ax {₁₊ n} (swap• ε , swap• ε) (comm {a = swap}) = refl , Eq.refl
  h-wd-ax {₁₊ n} (swap• ε , swap• ε) (comm {a = a ₛ}) = trans left-unit (sym right-unit) , Eq.refl
  h-wd-ax {₁₊ n} (swap• swap• ε , swap• ε) (comm {a = swap}) = refl , Eq.refl
  h-wd-ax {₁₊ n} (swap• swap• swap• c , swap• ε) (comm {a = swap}) = trans left-unit (sym right-unit) , Eq.refl
  ...
  h-wd-ax {₁₊ zero} (swap• swap• ε , swap• swap• ε) (comm {a = swap}) = sym (axiom (mid (comm swap swap))) , Eq.refl
  h-wd-ax {₁₊ zero} (swap• swap• ε , swap• swap• ε) (comm {a = () ₛ})




  h-wd-ax {n@(suc ( ₁₊ nn))} (swap• swap• c , ε) (congₛ (comm {a = a})) with ler5b (c) [ a ]ʷ | lel5b {nn} c [ a ]ʷ
  ... | (w6 , c6 , pr) | (w8 , c8 , pr8) = 
  ...
  h-wd-ax {n@(₁₊ nn)} (swap• c@(swap• c0) , ε) (comm {a = a}) =
  ...
  nfp-n+2 : (n : ℕ) → NormalFormWithoutInverse (pres (₂₊ n))
